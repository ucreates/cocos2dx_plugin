...
import Foundation
import UIKit
open class WebViewiOSPlugin: NSObject, UIWebViewDelegate {
    var view: UIWebView!
    @objc
    open func create(_ url: String, left: CGFloat, top: CGFloat, right: CGFloat, bottom: CGFloat) -> Void {
        ...
        let controller: UIViewController = ViewControlleriOSPlugin.getInstance()
    ...
    open func webView(_ webView: UIWebView, shouldStartLoadWith request: URLRequest, navigationType: UIWebViewNavigationType) -> Bool {
        print(TagiOSPlugin.UNITY_PLUGIN_IDENTIFIER + (request.url?.absoluteString)!)
        return true
    }
}
