...
RenderState::RenderState() : _texture(nullptr), _hash(0), _hashDirty(true), _parent(nullptr) {
    _state = StateBlock::create();
    CC_SAFE_RETAIN(_state);
}
RenderState::~RenderState() { CC_SAFE_RELEASE(_state); }
...
std::string RenderState::getName() const { return _name; }
...
Texture2D* RenderState::getTexture() const { return _texture; }
void RenderState::bind(Pass* pass) {
    CC_ASSERT(pass);
    if (_texture) GL::bindTexture2D(_texture->getName());
...
RenderState::StateBlock* RenderState::getStateBlock() const { return _state; }
...
static bool parseBoolean(const std::string& value) { return (value.compare("true") == 0); }
