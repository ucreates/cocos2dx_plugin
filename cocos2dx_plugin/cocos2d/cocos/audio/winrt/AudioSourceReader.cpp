...
// AudioFileReader
AudioSourceReader::AudioSourceReader() : _isStreaming(false), _filePath(""), _audioSize(0), _bytesRead(0), _isDirty(false) { memset(&_wfx, 0, sizeof(_wfx)); }
AudioSourceReader::~AudioSourceReader() { flushChunks(); }
...
// WAVFileReader
WAVReader::WAVReader() : _streamer(nullptr) {}
WAVReader::~WAVReader() {}
...
void WAVReader::seekTo(const float ratio) { AudioSourceReader::seekTo(ratio); }
// MP3Reader
MP3Reader::MP3Reader() : _mappedWavFile(""), _largeFileSupport(true) {}
MP3Reader::~MP3Reader() {}
...
void MP3Reader::seekTo(const float ratio) { AudioSourceReader::seekTo(ratio); }
...
void OGGReader::seekTo(const float ratio) { AudioSourceReader::seekTo(ratio); }
#endif
