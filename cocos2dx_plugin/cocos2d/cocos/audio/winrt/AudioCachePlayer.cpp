...
// AudioCache
AudioCache::AudioCache() : _isReady(false), _retry(false), _fileFullPath(""), _srcReader(nullptr), _fileFormat(FileFormat::UNKNOWN) {
    _callbacks.clear();
...
AudioPlayer::~AudioPlayer() { free(); }
void AudioPlayer::stop() { _stop(); }
void AudioPlayer::pause() { _stop(true); }
...
void AudioPlayer::resume() { _play(true); }
bool AudioPlayer::isInError() { return _criticalError; }
...
void AudioPlayer::OnBufferStart(void* pBufferContext) { UNREFERENCED_PARAMETER(pBufferContext); }
