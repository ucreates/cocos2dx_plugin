...
void AudioEngineCallbacks::Initialize(Audio* audio) { m_audio = audio; }
...
Audio::Audio() : m_backgroundID(0), m_soundEffctVolume(1.0f), m_backgroundMusicVolume(1.0f) {}
...
void Audio::Start() {
    ...
    if (!m_backgroundFile.empty()) PlayBackgroundMusic(m_backgroundFile.c_str(), m_backgroundLoop);
}
...
bool Audio::IsBackgroundMusicPlaying() { return IsSoundEffectStarted(m_backgroundID) && !IsSoundEffectPaused(m_backgroundID); }
...
float Audio::GetBackgroundVolume() { return m_backgroundMusicVolume; }
void Audio::SetSoundEffectVolume(float volume) {
    ...
    for (iter = m_soundEffects.begin(); iter != m_soundEffects.end(); iter++) {
        if (iter->first != m_backgroundID) iter->second.m_soundEffectSourceVoice->SetVolume(m_soundEffctVolume);
    }
}
float Audio::GetSoundEffectVolume() { return m_soundEffctVolume; }
...
void Audio::PauseAllSoundEffects() {
    ...
    EffectList::iterator iter;
    for (iter = m_soundEffects.begin(); iter != m_soundEffects.end(); iter++) {
        if (iter->first != m_backgroundID) PauseSoundEffect(iter->first);
    }
}
void Audio::ResumeAllSoundEffects() {
    ...
    EffectList::iterator iter;
    for (iter = m_soundEffects.begin(); iter != m_soundEffects.end(); iter++) {
        if (iter->first != m_backgroundID) ResumeSoundEffect(iter->first);
    }
}
...
bool Audio::IsSoundEffectStarted(unsigned int sound) {
    if (m_soundEffects.end() == m_soundEffects.find(sound)) return false;
...
void Audio::PreloadSoundEffect(const char* pszFilePath, bool isMusic) {
    ...
    if (isMusic) {
        ...
        if (FAILED(m_musicEngine->CreateSourceVoice(&m_soundEffects[sound].m_soundEffectSourceVoice, &wfx, 0, 1.0f, &m_voiceContext, &sends))) {
            m_engineExperiencedCriticalError = true;
        }
        ...
        sends.SendCount = 1;
        sends.pSends = descriptors;
        if (FAILED(m_soundEffectEngine->CreateSourceVoice(&m_soundEffects[sound].m_soundEffectSourceVoice, &wfx, 0, 1.0f, &m_voiceContext, &sends, nullptr))) {
            m_engineExperiencedCriticalError = true;
        }
        // fix bug: set a initial volume
...
void Audio::UnloadSoundEffect(unsigned int sound) {
    ...
    if (m_soundEffects[sound].m_soundEffectBufferData) delete[] m_soundEffects[sound].m_soundEffectBufferData;
...
void Audio::RemoveFromList(unsigned int sound) { m_soundEffects.erase(sound); }
