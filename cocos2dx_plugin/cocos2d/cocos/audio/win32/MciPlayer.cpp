...
MciPlayer::MciPlayer() : _wnd(NULL), _dev(0L), _soundID(0), _times(0), _playing(false), strExt("") {
    if (!s_hInstance) {
    ...
    _wnd = CreateWindowEx(WS_EX_APPWINDOW,                         // Extended Style For The Window
...
UINT MciPlayer::GetSoundID() { return _soundID; }
//////////////////////////////////////////////////////////////////////////
// private member
//////////////////////////////////////////////////////////////////////////
...
LRESULT WINAPI _SoundPlayProc(HWND hWnd, UINT Msg, WPARAM wParam, LPARAM lParam) {
    ...
    if (MM_MCINOTIFY == Msg && MCI_NOTIFY_SUCCESSFUL == wParam && (pPlayer = (MciPlayer*)GetWindowLongPtr(hWnd, GWLP_USERDATA))) {
        if (pPlayer->_times) {
            --pPlayer->_times;
        }
        if (pPlayer->_times) {
