...
void BaseLight::onEnter() {
    ...
    if (scene) {
        ...
        if (iter == lights.end()) lights.push_back(this);
    }
    Node::onEnter();
}
void BaseLight::onExit() {
    ...
    if (scene) {
        auto &lights = scene->_lights;
        auto iter = std::find(lights.begin(), lights.end(), this);
        if (iter != lights.end()) lights.erase(iter);
    }
    Node::onExit();
}
...
BaseLight::BaseLight() : _intensity(1.0f), _lightFlag(LightFlag::LIGHT0), _enabled(true) {}
BaseLight::~BaseLight() {}
////////////////////////////////////////////////////////////////////
...
void DirectionLight::setDirection(const Vec3 &dir) { setRotationFromDirection(dir); }
...
void SpotLight::setDirection(const Vec3 &dir) { setRotationFromDirection(dir); }
