...
void LoadingBarReader::destroyInstance() { CC_SAFE_DELETE(instanceLoadingBar); }
...
Offset<Table> LoadingBarReader::createOptionsWithFlatBuffers(const tinyxml2::XMLElement* objectData, flatbuffers::FlatBufferBuilder* builder) {
    ...
    auto options = CreateLoadingBarOptions(*builder, widgetOptions, CreateResourceData(*builder, builder->CreateString(path), builder->CreateString(plistFile), resourceType), percent, direction);
