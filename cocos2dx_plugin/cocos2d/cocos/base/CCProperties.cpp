...
Properties::Properties() : _variables(nullptr), _dirPath(nullptr), _parent(nullptr), _dataIdx(nullptr), _data(nullptr) { _properties.reserve(32); }
Properties::Properties(const Properties& copy)
    : _namespace(copy._namespace), _id(copy._id), _parentID(copy._parentID), _properties(copy._properties), _variables(nullptr), _dirPath(nullptr), _parent(copy._parent), _dataIdx(copy._dataIdx), _data(copy._data) {
...
Properties::Properties(Data* data, ssize_t* dataIdx) : _variables(NULL), _dirPath(NULL), _parent(NULL), _dataIdx(dataIdx), _data(data) {
    readProperties();
    rewind();
}
Properties::Properties(Data* data, ssize_t* dataIdx, const std::string& name, const char* id, const char* parentID, Properties* parent) : _namespace(name), _variables(NULL), _dirPath(NULL), _parent(parent), _dataIdx(dataIdx), _data(data) {
    if (id) {
        _id = id;
    }
    if (parentID) {
...
static bool isVariable(const char* str, char* outName, size_t outSize) {
    ...
    if (len > 3 && str[0] == '$' && str[1] == '{' && str[len - 1] == '}') {
        size_t size = len - 3;
        if (size > (outSize - 1)) size = outSize - 1;
    ...
    while (true) {
        ...
        if (comment) {
            ...
            if (strncmp(line, "*/", 2) == 0)
                ...
                if (len >= 2 && strncmp(line + (len - 2), "*/", 2) == 0) comment = false;
            }
                ...
                if (value != NULL && value[0] == '{') {
                    ...
                    if (rc != NULL) {
                        ...
                        if (c == '{') {
                            ...
                            // Back up from fgetc()
                            if (seekFromCurrent(-1) == false) CCLOGERROR("Failed to seek backwards a single character after testing if the next line starts with '{'.");
...
bool Properties::eof() { return (*_dataIdx >= _data->_size); }
...
void Properties::mergeWith(Properties* overrides) {
    ...
    while (overridesNamespace) {
        ...
        while (derivedNamespace) {
            if (strcmp(derivedNamespace->getNamespace(), overridesNamespace->getNamespace()) == 0 && strcmp(derivedNamespace->getId(), overridesNamespace->getId()) == 0) {
...
Properties* Properties::getNamespace(const char* id, bool searchNames, bool recurse) const {
    ...
    for (std::vector<Properties*>::const_iterator it = _namespaces.begin(); it < _namespaces.end(); ++it) {
        Properties* p = *it;
        if (strcmp(searchNames ? p->_namespace.c_str() : p->_id.c_str(), id) == 0) return p;
        if (recurse) {
            // Search recursively.
...
const char* Properties::getNamespace() const { return _namespace.c_str(); }
const char* Properties::getId() const { return _id.c_str(); }
...
const char* Properties::getString(const char* name, const char* defaultValue) const {
    ...
    if (value) {
        ...
        if (isVariable(value, variable, 256)) return getVariable(variable, defaultValue);
        return value;
    }
    return defaultValue;
}
...
bool Properties::getMat4(const char* name, Mat4* out) const {
    ...
    if (valueString) {
        float m[16];
        int scanned;
        scanned = sscanf(valueString, "%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f", &m[0], &m[1], &m[2], &m[3], &m[4], &m[5], &m[6], &m[7], &m[8], &m[9], &m[10], &m[11], &m[12], &m[13], &m[14], &m[15]);
        if (scanned != 16) {
...
bool Properties::getVec2(const char* name, Vec2* out) const { return parseVec2(getString(name), out); }
...
bool Properties::getQuaternionFromAxisAngle(const char* name, Quaternion* out) const { return parseAxisAngle(getString(name), out); }
...
const char* Properties::getVariable(const char* name, const char* defaultValue) const {
    ...
    if (_variables) {
        for (size_t i = 0, count = _variables->size(); i < count; ++i) {
            ...
            if (prop.name == name) return prop.value.c_str();
        }
    }
...
void Properties::setVariable(const char* name, const char* value) {
    ...
    if (prop) {
        ...
        if (!_variables) _variables = new (std::nothrow) std::vector<Property>();
...
bool Properties::parseAxisAngle(const char* str, Quaternion* out) {
    if (str) {
        ...
        if (sscanf(str, "%f,%f,%f,%f", &x, &y, &z, &theta) == 4) {
            if (out) out->set(Vec3(x, y, z), MATH_DEG_TO_RAD(theta));
            return true;
        } else {
    ...
    if (str) {
        if (strlen(str) == 7 && str[0] == '#') {
            ...
            if (sscanf(str + 1, "%x", &color) == 1) {
                if (out) out->set(Vec3::fromColor(color));
                return true;
            } else {
                // Invalid format
...
bool Properties::parseColor(const char* str, Vec4* out) {
    if (str) {
        if (strlen(str) == 9 && str[0] == '#') {
            ...
            unsigned int color;
            if (sscanf(str + 1, "%x", &color) == 1) {
                if (out) out->set(Vec4::fromColor(color));
                return true;
            } else {
                // Invalid format
