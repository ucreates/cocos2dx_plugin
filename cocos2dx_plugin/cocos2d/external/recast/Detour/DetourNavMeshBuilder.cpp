...
static int createBVTree(const unsigned short* verts, const int /*nverts*/, const unsigned short* polys, const int npolys, const int nvp, const float cs, const float ch, const int /*nnodes*/, dtBVNode* nodes) {
    // Build tree
...
bool dtCreateNavMeshData(dtNavMeshCreateParams* params, unsigned char** outData, int* outDataSize) {
    ...
    if (params->offMeshConCount > 0) {
        ...
        for (int i = 0; i < params->offMeshConCount; ++i) {
            ...
            if (offMeshConClass[i * 2 + 0] == 0xff) {
                if (p0[1] < bmin[1] || p0[1] > bmax[1]) offMeshConClass[i * 2 + 0] = 0;
            }
    ...
    dtMeshHeader* header = (dtMeshHeader*)d;
    d += headerSize;
    float* navVerts = (float*)d;
    d += vertsSize;
    dtPoly* navPolys = (dtPoly*)d;
    d += polysSize;
    d += linksSize;
    dtPolyDetail* navDMeshes = (dtPolyDetail*)d;
    d += detailMeshesSize;
    float* navDVerts = (float*)d;
    d += detailVertsSize;
    unsigned char* navDTris = (unsigned char*)d;
    d += detailTrisSize;
    dtBVNode* navBvtree = (dtBVNode*)d;
    d += bvTreeSize;
    ...
    if (params->buildBvTree) {
        createBVTree(params->verts, params->vertCount, params->polys, params->polyCount, nvp, params->cs, params->ch, params->polyCount * 2, navBvtree);
    }
    ...
    for (int i = 0; i < params->offMeshConCount; ++i) {
        ...
        if (offMeshConClass[i * 2 + 0] == 0xff) {
            ...
            if (params->offMeshConUserID) con->userId = params->offMeshConUserID[i];
            n++;
        }
    }
    dtFree(offMeshConClass);
...
bool dtNavMeshDataSwapEndian(unsigned char* data, const int /*dataSize*/) {
    ...
    for (int i = 0; i < header->offMeshConCount; ++i) {
        ...
        for (int j = 0; j < 6; ++j) dtSwapEndian(&con->pos[j]);
