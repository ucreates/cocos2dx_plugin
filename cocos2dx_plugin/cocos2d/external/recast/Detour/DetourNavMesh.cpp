...
int dtNavMesh::findConnectingPolys(const float* va, const float* vb, const dtMeshTile* tile, int side, dtPolyRef* con, float* conarea, int maxcon) const {
    ...
    for (int i = 0; i < tile->header->polyCount; ++i) {
        ...
        for (int j = 0; j < nv; ++j) {
            ...
            if (dtAbs(apos - bpos) > 0.01f) continue;
...
void dtNavMesh::connectExtLinks(dtMeshTile* tile, dtMeshTile* target, int side) {
    ...
    for (int i = 0; i < tile->header->polyCount; ++i) {
        ...
        for (int j = 0; j < nv; ++j) {
            ...
            if ((poly->neis[j] & DT_EXT_LINK) == 0) continue;
...
void dtNavMesh::connectExtOffMeshLinks(dtMeshTile* tile, dtMeshTile* target, int side) {
    ...
    for (int i = 0; i < target->header->offMeshConCount; ++i) {
        ...
        if (targetPoly->firstLink == DT_NULL_LINK) continue;
        ...
        if (dtSqr(nearestPt[0] - p[0]) + dtSqr(nearestPt[2] - p[2]) > dtSqr(targetCon->rad)) continue;
...
void dtNavMesh::baseOffMeshLinks(dtMeshTile* tile) {
    ...
    for (int i = 0; i < tile->header->offMeshConCount; ++i) {
        ...
        if (!ref) continue;
        // findNearestPoly may return too optimistic results, further check to make sure.
        if (dtSqr(nearestPt[0] - p[0]) + dtSqr(nearestPt[2] - p[2]) > dtSqr(con->rad)) continue;
        // Make sure the location is on current mesh.
    ...
    if (tile->bvTree) {
        ...
        while (node < end) {
            ...
            if (isLeafNode && overlap) {
                if (n < maxPolys) polys[n++] = base | (dtPolyRef)node->i;
            }
        ...
        for (int i = 0; i < tile->header->polyCount; ++i) {
            ...
            if (dtOverlapBounds(qmin, qmax, bmin, bmax)) {
                if (n < maxPolys) polys[n++] = base | (dtPolyRef)i;
            }
        }
        return n;
    }
...
dtStatus dtNavMesh::addTile(unsigned char* data, int dataSize, int flags, dtTileRef lastRef, dtTileRef* result) {
    ...
    if (getTileAt(header->x, header->y, header->layer)) return DT_FAILURE;
    // Allocate a tile.
    dtMeshTile* tile = 0;
    if (!lastRef) {
        if (m_nextFree) {
        ...
        if (tileIndex >= m_maxTiles) return DT_FAILURE | DT_OUT_OF_MEMORY;
    ...
    if (!bvtreeSize) tile->bvTree = 0;
    // Build links freelist
    tile->linksFreeList = 0;
    tile->links[header->maxLinkCount - 1].next = DT_NULL_LINK;
    for (int i = 0; i < header->maxLinkCount - 1; ++i) tile->links[i].next = i + 1;
    // Init tile.
    tile->header = header;
    tile->data = data;
...
const dtMeshTile* dtNavMesh::getTileByRef(dtTileRef ref) const {
    ...
    if ((int)tileIndex >= m_maxTiles) return 0;
...
dtStatus dtNavMesh::removeTile(dtTileRef ref, unsigned char** data, int* dataSize) {
    ...
    unsigned int tileIndex = decodePolyIdTile((dtPolyRef)ref);
    unsigned int tileSalt = decodePolyIdSalt((dtPolyRef)ref);
    if ((int)tileIndex >= m_maxTiles) return DT_FAILURE | DT_INVALID_PARAM;
    ...
    for (int i = 0; i < 8; ++i) {
        ...
        for (int j = 0; j < nneis; ++j) unconnectExtLinks(neis[j], tile);
    }
    // Reset tile.
...
dtStatus dtNavMesh::restoreTileState(dtMeshTile* tile, const unsigned char* data, const int maxDataSize) {
    ...
    if (tileState->magic != DT_NAVMESH_STATE_MAGIC) return DT_FAILURE | DT_WRONG_MAGIC;
    if (tileState->version != DT_NAVMESH_STATE_VERSION) return DT_FAILURE | DT_WRONG_VERSION;
    if (tileState->ref != getTileRef(tile)) return DT_FAILURE | DT_INVALID_PARAM;
...
dtStatus dtNavMesh::getOffMeshConnectionPolyEndPoints(dtPolyRef prevRef, dtPolyRef polyRef, float* startPos, float* endPos) const {
    ...
    if (poly->getType() != DT_POLYTYPE_OFFMESH_CONNECTION) return DT_FAILURE;
...
const dtOffMeshConnection* dtNavMesh::getOffMeshConnectionByRef(dtPolyRef ref) const {
    ...
    const dtPoly* poly = &tile->polys[ip];
    // Make sure that the current poly is indeed off-mesh link.
    if (poly->getType() != DT_POLYTYPE_OFFMESH_CONNECTION) return 0;
