...
dtPathQueue::dtPathQueue() : m_nextHandle(1), m_maxPathSize(0), m_queueHead(0), m_navquery(0) {
    for (int i = 0; i < MAX_QUEUE; ++i) m_queue[i].path = 0;
}
...
bool dtPathQueue::init(const int maxPathSize, const int maxSearchNodeCount, dtNavMesh* nav) {
    ...
    if (dtStatusFailed(m_navquery->init(nav, maxSearchNodeCount))) return false;
    ...
    for (int i = 0; i < MAX_QUEUE; ++i) {
        ...
        if (!m_queue[i].path) return false;
    }
    m_queueHead = 0;
    return true;
}
...
dtPathQueueRef dtPathQueue::request(dtPolyRef startRef, dtPolyRef endRef, const float* startPos, const float* endPos, const dtQueryFilter* filter) {
    // Find empty slot
    int slot = -1;
    for (int i = 0; i < MAX_QUEUE; ++i) {
    ...
    // Could not find slot.
    if (slot == -1) return DT_PATHQ_INVALID;
...
dtStatus dtPathQueue::getRequestStatus(dtPathQueueRef ref) const {
    for (int i = 0; i < MAX_QUEUE; ++i) {
        if (m_queue[i].ref == ref) return m_queue[i].status;
    }
    return DT_FAILURE;
}
