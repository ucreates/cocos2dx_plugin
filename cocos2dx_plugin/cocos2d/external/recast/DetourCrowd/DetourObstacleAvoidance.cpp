...
dtObstacleAvoidanceDebugData::dtObstacleAvoidanceDebugData() : m_nsamples(0), m_maxSamples(0), m_vel(0), m_ssize(0), m_pen(0), m_vpen(0), m_vcpen(0), m_spen(0), m_tpen(0) {}
...
static void normalizeArray(float* arr, const int n) {
    ...
    for (int i = 0; i < n; ++i) arr[i] = dtClamp((arr[i] - minPen) * s, 0.0f, 1.0f);
}
...
dtObstacleAvoidanceQuery::dtObstacleAvoidanceQuery() : m_maxCircles(0), m_circles(0), m_ncircles(0), m_maxSegments(0), m_segments(0), m_nsegments(0) {}
...
void dtObstacleAvoidanceQuery::addCircle(const float* pos, const float rad, const float* vel, const float* dvel) {
    if (m_ncircles >= m_maxCircles) return;
...
void dtObstacleAvoidanceQuery::addSegment(const float* p, const float* q) {
    if (m_nsegments >= m_maxSegments) return;
...
float dtObstacleAvoidanceQuery::processSample(const float* vcand, const float cs, const float* pos, const float rad, const float* vel, const float* dvel, const float minPenalty, dtObstacleAvoidanceDebugData* debug) {
    ...
    if (tThresold - m_params.horizTime > -FLT_EPSILON) return minPenalty;  // already too much
    ...
    for (int i = 0; i < m_ncircles; ++i) {
        ...
        if (!sweepCircleCircle(pos, rad, vab, cir->p, cir->rad, htmin, htmax)) continue;
    ...
    for (int i = 0; i < m_nsegments; ++i) {
        ...
        if (seg->touch) {
            ...
            if (dtVdot2D(snorm, vcand) < 0.0f) continue;
            // Else immediate collision.
            htmin = 0.0f;
        } else {
            if (!isectRaySeg(pos, vcand, seg->p, seg->q, htmin)) continue;
        }
    ...
    if (debug) debug->addSample(vcand, cs, penalty, vpen, vcpen, spen, tpen);
    return penalty;
}
