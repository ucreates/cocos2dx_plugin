...
static bool buildMeshAdjacency(unsigned short* polys, const int npolys, const int nverts, const int vertsPerPoly) {
    ...
    for (int i = 0; i < nverts; i++) firstEdge[i] = RC_MESH_NULL_IDX;
...
static unsigned short addVertex(unsigned short x, unsigned short y, unsigned short z, unsigned short* verts, int* firstVert, int* nextVert, int& nv) {
    ...
    while (i != -1) {
        ...
        if (v[0] == x && (rcAbs(v[1] - y) <= 2) && v[2] == z) return (unsigned short)i;
        i = nextVert[i];  // next
    }
...
static int triangulate(int n, const int* verts, int* indices, int* tris) {
    ...
    for (int i = 0; i < n; i++) {
        int i1 = next(i, n);
        int i2 = next(i1, n);
        if (diagonal(i, i2, n, verts, indices)) indices[i1] |= 0x80000000;
    }
    while (n > 3) {
        int minLen = -1;
        int mini = -1;
...
static int getPolyMergeValue(unsigned short* pa, unsigned short* pb, const unsigned short* verts, int& ea, int& eb, const int nvp) {
    ...
    for (int i = 0; i < na; ++i) {
        ...
        if (va0 > va1) rcSwap(va0, va1);
        for (int j = 0; j < nb; ++j) {
            unsigned short vb0 = pb[j];
            unsigned short vb1 = pb[(j + 1) % nb];
            if (vb0 > vb1) rcSwap(vb0, vb1);
            if (va0 == vb0 && va1 == vb1) {
                ea = i;
                eb = j;
                break;
...
static bool removeVertex(rcContext* ctx, rcPolyMesh& mesh, const unsigned short rem, const int maxTris) {
    ...
    for (int i = 0; i < mesh.npolys; ++i) {
        ...
        if (hasRem) {
            ...
            if (p != p2) memcpy(p, p2, sizeof(unsigned short) * nvp);
    ...
    if (nvp > 3) {
        for (;;) {
            ...
            if (bestMergeVal > 0) {
                ...
                if (pb != last) memcpy(pb, last, sizeof(unsigned short) * nvp);
    ...
    for (int i = 0; i < npolys; ++i) {
        ...
        for (int j = 0; j < nvp; ++j) p[j] = polys[i * nvp + j];
        ...
        if (nvp > 3) {
            for (;;) {
                ...
                if (bestMergeVal > 0) {
                    ...
                    if (pb != lastPoly) memcpy(pb, lastPoly, sizeof(unsigned short) * nvp);
                    npolys--;
                } else {
                    // Could not merge any polygons, stop.
                    break;
