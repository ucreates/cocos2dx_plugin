...
bool rcBuildHeightfieldLayers(rcContext* ctx, rcCompactHeightfield& chf, const int borderSize, const int walkableHeight, rcHeightfieldLayerSet& lset) {
    ...
    for (int y = borderSize; y < h - borderSize; ++y) {
        ...
        for (int x = borderSize; x < w - borderSize; ++x) {
            const rcCompactCell& c = chf.cells[x + y * w];
            for (int i = (int)c.index, ni = (int)(c.index + c.count); i < ni; ++i) {
                if (srcReg[i] != 0xff) srcReg[i] = sweeps[srcReg[i]].id;
            }
        }
    }
            ...
            for (int i = (int)c.index, ni = (int)(c.index + c.count); i < ni; ++i) {
                ...
                for (int dir = 0; dir < 4; ++dir) {
                    if (rcGetCon(s, dir) != RC_NOT_CONNECTED) {
                        ...
                        if (rai != 0xff && rai != ri) addUnique(regs[ri].neis, regs[ri].nneis, rai);
                    }
                }
            }
    ...
    for (int i = 0; i < nregs; ++i) {
        ...
        while (nstack) {
            ...
            nstack--;
            for (int j = 0; j < nstack; ++j) stack[j] = stack[j + 1];
            ...
            for (int j = 0; j < nneis; ++j) {
                ...
                if (nstack < MAX_STACK) {
                    ...
                    for (int k = 0; k < regn.nlayers; ++k) addUnique(root.layers, root.nlayers, regn.layers[k]);
        ...
        for (;;) {
            ...
            for (int j = 0; j < nregs; ++j) {
                ...
                if (rj.layerId == oldId) {
                    ...
                    for (int k = 0; k < rj.nlayers; ++k) addUnique(ri.layers, ri.nlayers, rj.layers[k]);
    ...
    layerId = 0;
    for (int i = 0; i < nregs; ++i) remap[regs[i].layerId] = 1;
    for (int i = 0; i < 256; ++i) {
        if (remap[i])
            remap[i] = layerId++;
        else
    ...
    // Remap ids.
    for (int i = 0; i < nregs; ++i) regs[i].layerId = remap[regs[i].layerId];
                    ...
                    for (int dir = 0; dir < 4; ++dir) {
                        if (rcGetCon(s, dir) != RC_NOT_CONNECTED) {
                            ...
                            if (chf.areas[ai] != RC_NULL_AREA && lid != alid) {
                                ...
                                if (as.y > hmin) layer->heights[idx] = rcMax(layer->heights[idx], (unsigned char)(as.y - hmin));
                            }
                            // Valid connection mask
                            if (chf.areas[ai] != RC_NULL_AREA && lid == alid) {
                                const int nx = ax - borderSize;
                                const int ny = ay - borderSize;
                                if (nx >= 0 && ny >= 0 && nx < lw && ny < lh) con |= (unsigned char)(1 << dir);
                            }
                        }
                    }
