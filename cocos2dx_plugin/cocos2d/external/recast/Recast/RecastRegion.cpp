...
static void calculateDistanceField(rcCompactHeightfield& chf, unsigned short* src, unsigned short& maxDist) {
    ...
    for (int i = 0; i < chf.spanCount; ++i) src[i] = 0xffff;
    // Mark boundary cells.
    for (int y = 0; y < h; ++y) {
        for (int x = 0; x < w; ++x) {
...
static void sortCellsByLevel(unsigned short startLevel, rcCompactHeightfield& chf, unsigned short* srcReg, unsigned int nbStacks, rcIntArray* stacks, unsigned short loglevelsPerStack)  // the levels per stack (2 in our case) as a bit shift
{
    const int w = chf.width;
    const int h = chf.height;
    startLevel = startLevel >> loglevelsPerStack;
    for (unsigned int j = 0; j < nbStacks; ++j) stacks[j].resize(0);
    // put all cells in the level range into the appropriate stacks
    for (int y = 0; y < h; ++y) {
        for (int x = 0; x < w; ++x) {
            const rcCompactCell& c = chf.cells[x + y * w];
            for (int i = (int)c.index, ni = (int)(c.index + c.count); i < ni; ++i) {
                if (chf.areas[i] == RC_NULL_AREA || srcReg[i] != 0) continue;
                int level = chf.dist[i] >> loglevelsPerStack;
                int sId = startLevel - level;
                if (sId >= (int)nbStacks) continue;
                if (sId < 0) sId = 0;
                stacks[sId].push(x);
                stacks[sId].push(y);
                stacks[sId].push(i);
...
static void removeAdjacentNeighbours(rcRegion& reg) {
    ...
    for (int i = 0; i < reg.connections.size() && reg.connections.size() > 1;) {
        ...
        if (reg.connections[i] == reg.connections[ni]) {
            // Remove duplicate
            for (int j = i; j < reg.connections.size() - 1; ++j) reg.connections[j] = reg.connections[j + 1];
            reg.connections.pop();
        } else
            ++i;
    }
...
static void replaceNeighbour(rcRegion& reg, unsigned short oldId, unsigned short newId) {
    ...
    for (int i = 0; i < reg.floors.size(); ++i) {
        if (reg.floors[i] == oldId) reg.floors[i] = newId;
    }
...
static bool mergeRegions(rcRegion& rega, rcRegion& regb) {
    ...
    for (int i = 0; i < rega.connections.size(); ++i) acon[i] = rega.connections[i];
    ...
    for (int i = 0, ni = acon.size(); i < ni - 1; ++i) rega.connections.push(acon[(insa + 1 + i) % ni]);
    for (int i = 0, ni = bcon.size(); i < ni - 1; ++i) rega.connections.push(bcon[(insb + 1 + i) % ni]);
    removeAdjacentNeighbours(rega);
    for (int j = 0; j < regb.floors.size(); ++j) addUniqueFloorRegion(rega, regb.floors[j]);
...
static void walkContour(int x, int y, int i, int dir, rcCompactHeightfield& chf, unsigned short* srcReg, rcIntArray& cont) {
    ...
    if (cont.size() > 1) {
        for (int j = 0; j < cont.size();) {
            ...
            if (cont[j] == cont[nj]) {
                for (int k = j; k < cont.size() - 1; ++k) cont[k] = cont[k + 1];
                cont.pop();
            } else
                ++j;
        }
...
static bool mergeAndFilterRegions(rcContext* ctx, int minRegionArea, int mergeRegionSize, unsigned short& maxRegionId, rcCompactHeightfield& chf, unsigned short* srcReg, rcIntArray& overlaps) {
    ...
    // Construct regions
    for (int i = 0; i < nreg; ++i) new (&regions[i]) rcRegion((unsigned short)i);
            ...
            if (mergeId != reg.id) {
                ...
                if (mergeRegions(target, reg)) {
                    ...
                    for (int j = 0; j < nreg; ++j) {
                        ...
                        if (regions[j].id == oldId) regions[j].id = mergeId;
    ...
    for (int i = 0; i < nreg; ++i)
        if (regions[i].overlap) overlaps.push(regions[i].id);
    for (int i = 0; i < nreg; ++i) regions[i].~rcRegion();
    rcFree(regions);
    return true;
}
...
static bool mergeAndFilterLayerRegions(rcContext* ctx, int minRegionArea, unsigned short& maxRegionId, rcCompactHeightfield& chf, unsigned short* srcReg, rcIntArray& overlaps) {
    const int w = chf.width;
    const int h = chf.height;
    const int nreg = maxRegionId + 1;
    rcRegion* regions = (rcRegion*)rcAlloc(sizeof(rcRegion) * nreg, RC_ALLOC_TEMP);
    ...
    for (int i = 0; i < nreg; ++i) regions[i].id = 0;
    ...
    for (int i = 1; i < nreg; ++i) {
        ...
        while (stack.size() > 0) {
            ...
            for (int j = 0; j < stack.size() - 1; ++j) stack[j] = stack[j + 1];
            ...
            for (int j = 0; j < ncons; ++j) {
                ...
                for (int k = 0; k < regn.floors.size(); ++k) addUniqueFloorRegion(root, regn.floors[k]);
    ...
    for (int i = 0; i < nreg; ++i) {
        if (regions[i].spanCount > 0 && regions[i].spanCount < minRegionArea && !regions[i].connectsToBorder) {
            ...
            for (int j = 0; j < nreg; ++j)
                if (regions[j].id == reg) regions[j].id = 0;
        }
    }
    // Compress region Ids.
    for (int i = 0; i < nreg; ++i) {
...
bool rcBuildDistanceField(rcContext* ctx, rcCompactHeightfield& chf) {
    ...
    // Blur
    if (boxBlur(chf, 1, src, dst) != src) rcSwap(src, dst);
    // Store distance.
    chf.dist = src;
...
bool rcBuildRegionsMonotone(rcContext* ctx, rcCompactHeightfield& chf, const int borderSize, const int minRegionArea, const int mergeRegionArea) {
        ...
        for (int x = borderSize; x < w - borderSize; ++x) {
            ...
            for (int i = (int)c.index, ni = (int)(c.index + c.count); i < ni; ++i) {
                ...
                if (rcGetCon(s, 0) != RC_NOT_CONNECTED) {
                    ...
                    const int ay = y + rcGetDirOffsetY(0);
                    const int ai = (int)chf.cells[ax + ay * w].index + rcGetCon(s, 0);
                    if ((srcReg[ai] & RC_BORDER_REG) == 0 && chf.areas[i] == chf.areas[ai]) previd = srcReg[ai];
                }
                if (!previd) {
                    previd = rid++;
    ...
    rcIntArray overlaps;
    chf.maxRegions = id;
    if (!mergeAndFilterRegions(ctx, minRegionArea, mergeRegionArea, chf.maxRegions, chf, srcReg, overlaps)) return false;
    // Monotone partitioning does not generate overlapping regions.
    ctx->stopTimer(RC_TIMER_BUILD_REGIONS_FILTER);
    // Store the result out.
    for (int i = 0; i < chf.spanCount; ++i) chf.spans[i].reg = srcReg[i];
...
bool rcBuildRegions(rcContext* ctx, rcCompactHeightfield& chf, const int borderSize, const int minRegionArea, const int mergeRegionArea) {
    ...
    for (int i = 0; i < NB_STACKS; ++i) lvlStacks[i].resize(1024);
    ...
    while (level > 0) {
        ...
        for (int j = 0; j < lvlStacks[sId].size(); j += 3) {
            ...
            if (i >= 0 && srcReg[i] == 0) {
                if (floodRegion(x, y, i, level, regionId, chf, srcReg, srcDist, stack)) regionId++;
            }
        }
...
bool rcBuildLayerRegions(rcContext* ctx, rcCompactHeightfield& chf, const int borderSize, const int minRegionArea) {
    ...
    rcIntArray overlaps;
    chf.maxRegions = id;
    if (!mergeAndFilterLayerRegions(ctx, minRegionArea, chf.maxRegions, chf, srcReg, overlaps)) return false;
    ctx->stopTimer(RC_TIMER_BUILD_REGIONS_FILTER);
    // Store the result out.
    for (int i = 0; i < chf.spanCount; ++i) chf.spans[i].reg = srcReg[i];
    ctx->stopTimer(RC_TIMER_BUILD_REGIONS);
