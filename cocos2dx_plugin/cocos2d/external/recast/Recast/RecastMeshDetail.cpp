...
static float distToPoly(int nvert, const float* verts, const float* p) {
    ...
    for (i = 0, j = nvert - 1; i < nvert; j = i++) {
        ...
        if (((vi[2] > p[2]) != (vj[2] > p[2])) && (p[0] < (vj[0] - vi[0]) * (p[2] - vi[2]) / (vj[2] - vi[2]) + vi[0])) c = !c;
...
static void delaunayHull(rcContext* ctx, const int npts, const float* pts, const int nhull, const int* hull, rcIntArray& tris, rcIntArray& edges) {
    ...
    for (int i = 0, j = nhull - 1; i < nhull; j = i++) addEdge(ctx, &edges[0], nedges, maxEdges, hull[j], hull[i], EV_HULL, EV_UNDEF);
    int currentEdge = 0;
    while (currentEdge < nedges) {
        if (edges[currentEdge * 4 + 2] == EV_UNDEF) completeFacet(ctx, pts, npts, &edges[0], nedges, maxEdges, nfaces, currentEdge);
        if (edges[currentEdge * 4 + 3] == EV_UNDEF) completeFacet(ctx, pts, npts, &edges[0], nedges, maxEdges, nfaces, currentEdge);
        currentEdge++;
    }
    // Create tris
    tris.resize(nfaces * 4);
    for (int i = 0; i < nfaces * 4; ++i) tris[i] = -1;
    for (int i = 0; i < nedges; ++i) {
        const int* e = &edges[i * 4];
        if (e[3] >= 0) {
            // Left face
...
static bool buildPolyDetail(rcContext* ctx, const float* in, const int nin, const float sampleDist, const float sampleMaxError, const rcCompactHeightfield& chf, const rcHeightPatch& hp, float* verts, int& nverts, rcIntArray& tris, rcIntArray& edges,
                            rcIntArray& samples) {
    ...
    int nhull = 0;
    nverts = 0;
    for (int i = 0; i < nin; ++i) rcVcopy(&verts[i * 3], &in[i * 3]);
    nverts = nin;
    edges.resize(0);
    tris.resize(0);
        ...
        for (int i = 0, j = nin - 1; i < nin; j = i++) {
            ...
            for (int k = 0; k < nidx - 1;) {
                ...
                if (maxi != -1 && maxd > rcSqr(sampleMaxError)) {
                    for (int m = nidx; m > k; --m) idx[m] = idx[m - 1];
                    idx[k + 1] = maxi;
                    nidx++;
                } else {
                    ++k;
...
static void getHeightDataSeedsFromVertices(const rcCompactHeightfield& chf, const unsigned short* poly, const int npoly, const unsigned short* verts, const int bs, rcHeightPatch& hp, rcIntArray& stack) {
    ...
    while (stack.size() > 0) {
        ...
        for (int dir = 0; dir < 4; ++dir) {
            ...
            if (ax < hp.xmin || ax >= (hp.xmin + hp.width) || ay < hp.ymin || ay >= (hp.ymin + hp.height)) continue;
            if (hp.data[ax - hp.xmin + (ay - hp.ymin) * hp.width] != 0) continue;
...
static void getHeightData(const rcCompactHeightfield& chf, const unsigned short* poly, const int npoly, const unsigned short* verts, const int bs, rcHeightPatch& hp, rcIntArray& stack, int region) {
    ...
    if (empty) getHeightDataSeedsFromVertices(chf, poly, npoly, verts, bs, hp, stack);
    ...
    while (head * 3 < stack.size()) {
        ...
        if (head >= RETRACT_SIZE) {
            head = 0;
            if (stack.size() > RETRACT_SIZE * 3) memmove(&stack[0], &stack[RETRACT_SIZE * 3], sizeof(int) * (stack.size() - RETRACT_SIZE * 3));
...
static unsigned char getEdgeFlags(const float* va, const float* vb, const float* vpoly, const int npoly) {
    ...
    for (int i = 0, j = npoly - 1; i < npoly; j = i++) {
        if (distancePtSeg2d(va, &vpoly[j * 3], &vpoly[i * 3]) < thrSqr && distancePtSeg2d(vb, &vpoly[j * 3], &vpoly[i * 3]) < thrSqr) return 1;
    }
    return 0;
}
