...
static void ioprintf(duFileIO* io, const char* format, ...) {
    ...
    va_end(ap);
    if (n > 0) io->write(line, sizeof(char) * n);
}
...
bool duDumpPolyMeshDetailToObj(rcPolyMeshDetail& dmesh, duFileIO* io) {
    ...
    for (int i = 0; i < dmesh.nmeshes; ++i) {
        ...
        for (unsigned int j = 0; j < ntris; ++j) {
            ioprintf(io, "f %d %d %d\n", (int)(bverts + tris[j * 4 + 0]) + 1, (int)(bverts + tris[j * 4 + 1]) + 1, (int)(bverts + tris[j * 4 + 2]) + 1);
        }
    }
    return true;
}
...
bool duDumpCompactHeightfield(struct rcCompactHeightfield& chf, duFileIO* io) {
    ...
    if (chf.cells) io->write(chf.cells, sizeof(rcCompactCell) * chf.width * chf.height);
    if (chf.spans) io->write(chf.spans, sizeof(rcCompactSpan) * chf.spanCount);
    if (chf.dist) io->write(chf.dist, sizeof(unsigned short) * chf.spanCount);
    if (chf.areas) io->write(chf.areas, sizeof(unsigned char) * chf.spanCount);
    return true;
}
