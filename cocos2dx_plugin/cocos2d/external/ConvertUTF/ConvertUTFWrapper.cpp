...
bool ConvertUTF8toWide(unsigned WideCharWidth, const std::string &Source, char *&ResultPtr, const UTF8 *&ErrorPtr) {
    ...
    if (WideCharWidth == 1) {
        ...
        result = ConvertUTF8toUTF16(&sourceStart, sourceStart + Source.size(), &targetStart, targetStart + 2 * Source.size(), flags);
        ...
        ConversionFlags flags = strictConversion;
        result = ConvertUTF8toUTF32(&sourceStart, sourceStart + Source.size(), &targetStart, targetStart + 4 * Source.size(), flags);
        if (result == conversionOK)
            ResultPtr = reinterpret_cast<char *>(targetStart);
        else
            ErrorPtr = sourceStart;
...
bool ConvertCodePointToUTF8(unsigned Source, char *&ResultPtr) {
    ...
    ConversionResult CR = ConvertUTF32toUTF8(&SourceStart, SourceEnd, &TargetStart, TargetEnd, strictConversion);
    if (CR != conversionOK) return false;
    ResultPtr = reinterpret_cast<char *>(TargetStart);
    return true;
}
bool hasUTF16ByteOrderMark(const char *S, size_t len) { return (len >= 2 && ((S[0] == '\xff' && S[1] == '\xfe') || (S[0] == '\xfe' && S[1] == '\xff'))); }
...
bool convertUTF16ToUTF8String(const std::u16string &utf16, std::string &Out) {
    ...
    if (Src[0] == UNI_UTF16_BYTE_ORDER_MARK_SWAPPED) {
        ...
        for (size_t I = 0, E = ByteSwapped.size(); I != E; ++I) ByteSwapped[I] = SwapByteOrder_16(ByteSwapped[I]);
        Src = &ByteSwapped[0];
