...
class Int128 {
   public:
    ...
    Int128 operator/(const Int128& rhs) const {
        if (rhs.lo == 0 && rhs.hi == 0) throw "Int128 operator/: divide by zero";
...
bool FindSegment(OutPt*& pp, bool UseFullInt64Range, IntPoint& pt1, IntPoint& pt2) {
    // OutPt1 & OutPt2 => the overlap segment (if the function returns true)
    if (!pp) return false;
    OutPt* pp2 = pp;
    IntPoint pt1a = pt1, pt2a = pt2;
    do {
        if (SlopesEqual(pt1a, pt2a, pp->Pt, pp->Prev->Pt, UseFullInt64Range) && SlopesEqual(pt1a, pt2a, pp->Pt, UseFullInt64Range) && GetOverlapSegment(pt1a, pt2a, pp->Pt, pp->Prev->Pt, pt1, pt2)) return true;
        pp = pp->Next;
    } while (pp != pp2);
    return false;
}
...
bool HorzSegmentsOverlap(const IntPoint& pt1a, const IntPoint& pt1b, const IntPoint& pt2a, const IntPoint& pt2b) {
...
void RangeTest(const IntPoint& Pt, bool& useFullRange) {
    if (useFullRange) {
        if (Pt.X > hiRange || Pt.Y > hiRange || -Pt.X > hiRange || -Pt.Y > hiRange) throw "Coordinate outside allowed range";
...
void ClipperBase::InsertLocalMinima(LocalMinima* newLm) {
    if (!m_MinimaList) {
        ...
        while (tmpLm->Next && (newLm->Y < tmpLm->Next->Y)) tmpLm = tmpLm->Next;
...
IntRect ClipperBase::GetBounds() {
    ...
    while (lm) {
        if (lm->LeftBound->Bot.Y > result.bottom) result.bottom = lm->LeftBound->Bot.Y;
        TEdge* e = lm->LeftBound;
        for (;;) {
            TEdge* bottomE = e;
            while (e->NextInLML) {
...
bool Clipper::Execute(ClipType clipType, Paths& solution, PolyFillType subjFillType, PolyFillType clipFillType) {
    ...
    if (m_HasOpenPaths) throw clipperException("Error: PolyTree struct is need for open path clipping.");
...
bool Clipper::ExecuteInternal() {
    ...
    if (succeeded) {
        ...
        for (PolyOutList::size_type i = 0; i < m_PolyOuts.size(); ++i) {
            OutRec* outRec = m_PolyOuts[i];
            if (outRec->Pts && !outRec->IsOpen) FixupOutPolygon(*outRec);
        }
...
OutPt* Clipper::AddLocalMinPoly(TEdge* e1, TEdge* e2, const IntPoint& Pt) {
    ...
    if (prevE && prevE->OutIdx >= 0 && (TopX(*prevE, Pt.Y) == TopX(*e, Pt.Y)) && SlopesEqual(*e, *prevE, m_UseFullRange) && (e->WindDelta != 0) && (prevE->WindDelta != 0)) {
...
void Clipper::InsertLocalMinimaIntoAEL(const cInt botY) {
    while (m_CurrentLM && (m_CurrentLM->Y == botY)) {
        ...
        if (lb->OutIdx >= 0 && lb->PrevInAEL && lb->PrevInAEL->Curr.X == lb->Bot.X && lb->PrevInAEL->OutIdx >= 0 && SlopesEqual(*lb->PrevInAEL, *lb, m_UseFullRange) && (lb->WindDelta != 0) && (lb->PrevInAEL->WindDelta != 0)) {
...
void Clipper::IntersectEdges(TEdge* e1, TEdge* e2, const IntPoint& Pt, bool protect) {
    // e1 will be to the Left of e2 BELOW the intersection. Therefore e1 is before
    // e2 in AEL except when e1 is being inserted at the intersection point ...
    bool e1stops = !protect && !e1->NextInLML && e1->Top.X == Pt.X && e1->Top.Y == Pt.Y;
    bool e2stops = !protect && !e2->NextInLML && e2->Top.X == Pt.X && e2->Top.Y == Pt.Y;
    ...
    if (e1->WindDelta == 0 || e2->WindDelta == 0) {
        ...
        if (e1->WindDelta == 0 && e2->WindDelta == 0) {
            if ((e1stops || e2stops) && e1Contributing && e2Contributing) AddLocalMaxPoly(e1, e2, Pt);
        }
        ...
        if (e1stops)
            if (e1->OutIdx < 0)
                ...
                throw clipperException("Error intersecting polylines");
        if (e2stops)
            if (e2->OutIdx < 0)
                DeleteFromAEL(e2);
            else
    ...
    if (e1Contributing && e2Contributing) {
        if (e1stops || e2stops || (e1Wc != 0 && e1Wc != 1) || (e2Wc != 0 && e2Wc != 1) || (e1->PolyTyp != e2->PolyTyp && m_ClipType != ctXor))
        ...
        if (e1->PolyTyp != e2->PolyTyp)
            ...
            switch (m_ClipType) {
                ...
                case ctUnion:
                    ...
                    break;
                case ctDifference:
                    if (((e1->PolyTyp == ptClip) && (e1Wc2 > 0) && (e2Wc2 > 0)) || ((e1->PolyTyp == ptSubject) && (e1Wc2 <= 0) && (e2Wc2 <= 0))) AddLocalMinPoly(e1, e2, Pt);
                    break;
                case ctXor:
                    AddLocalMinPoly(e1, e2, Pt);
            }
    ...
    do {
        ...
        while (e->NextInSEL) {
            ...
            if (e->Curr.X > eNext->Curr.X) {
                if (!IntersectPoint(*e, *eNext, Pt, m_UseFullRange) && e->Curr.X > eNext->Curr.X + 1) throw clipperException("Intersection error");
                if (Pt.Y > botY) {
                    Pt.Y = botY;
...
void Clipper::ProcessEdgesAtTopOfScanbeam(const cInt topY) {
    ...
    while (e) {
        if (IsIntermediate(e, topY)) {
            ...
            TEdge* ePrev = e->PrevInAEL;
            TEdge* eNext = e->NextInAEL;
            if (ePrev && ePrev->Curr.X == e->Bot.X && ePrev->Curr.Y == e->Bot.Y && op && ePrev->OutIdx >= 0 && ePrev->Curr.Y > ePrev->Top.Y && SlopesEqual(*e, *ePrev, m_UseFullRange) && (e->WindDelta != 0) && (ePrev->WindDelta != 0)) {
                OutPt* op2 = AddOutPt(ePrev, e->Bot);
                AddJoin(op, op2, e->Top);
            } else if (eNext && eNext->Curr.X == e->Bot.X && eNext->Curr.Y == e->Bot.Y && op && eNext->OutIdx >= 0 && eNext->Curr.Y > eNext->Top.Y && SlopesEqual(*e, *eNext, m_UseFullRange) && (e->WindDelta != 0) && (eNext->WindDelta != 0)) {
...
void Clipper::FixupOutPolygon(OutRec& outrec) {
    ...
    for (;;) {
        ...
        if ((pp->Pt == pp->Next->Pt) || (pp->Pt == pp->Prev->Pt) || (SlopesEqual(pp->Prev->Pt, pp->Pt, pp->Next->Pt, m_UseFullRange) && (!m_PreserveCollinear || !Pt2IsBetweenPt1AndPt3(pp->Prev->Pt, pp->Pt, pp->Next->Pt)))) {
            lastOK = 0;
            OutPt* tmp = pp;
        ...
        if (outRec1 == outRec2) {
            ...
            if (m_UsingPolyTree)
                for (PolyOutList::size_type j = 0; j < m_PolyOuts.size() - 1; j++) {
                    ...
                    if (!oRec->Pts || ParseFirstLeft(oRec->FirstLeft) != outRec1 || oRec->IsHole == outRec1->IsHole) continue;
...
void ClipperOffset::AddPath(const Path& path, JoinType joinType, EndType endType) {
    ...
    for (int i = 1; i <= highI; i++)
        if (newNode->Contour[j] != path[i]) {
            ...
            if (path[i].Y > newNode->Contour[k].Y || (path[i].Y == newNode->Contour[k].Y && path[i].X < newNode->Contour[k].X)) k = j;
        }
    if ((endType == etClosedPolygon && j < 2) || (endType != etClosedPolygon && j < 0)) {
        delete newNode;
        return;
    }
    ...
    if (m_lowest.X < 0)
        ...
        if (newNode->Contour[k].Y > ip.Y || (newNode->Contour[k].Y == ip.Y && newNode->Contour[k].X < ip.X)) m_lowest = IntPoint(m_polyNodes.ChildCount() - 1, k);
    }
}
//------------------------------------------------------------------------------
...
void ClipperOffset::FixOrientations() {
    ...
    if (m_lowest.X >= 0 && !Orientation(m_polyNodes.Childs[(int)m_lowest.X]->Contour)) {
        for (int i = 0; i < m_polyNodes.ChildCount(); ++i) {
            ...
            if (node.m_endtype == etClosedPolygon || (node.m_endtype == etClosedLine && Orientation(node.Contour))) ReversePath(node.Contour);
        }
    } else {
        for (int i = 0; i < m_polyNodes.ChildCount(); ++i) {
            PolyNode& node = *m_polyNodes.Childs[i];
...
void AddPolyNodeToPolygons(const PolyNode& polynode, NodeType nodetype, Paths& paths) {
    ...
    for (int i = 0; i < polynode.ChildCount(); ++i) AddPolyNodeToPolygons(*polynode.Childs[i], nodetype, paths);
}
//------------------------------------------------------------------------------
...
void OffsetPaths(const Paths& in_polys, Paths& out_polys, double delta, JoinType jointype, EndType_ endtype, double limit) {
