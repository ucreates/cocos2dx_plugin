...
#define FLATBUFFERS_TD(ENUM, IDLTYPE, CTYPE, JTYPE, GTYPE, NTYPE) sizeof(CTYPE),
    FLATBUFFERS_GEN_TYPES(FLATBUFFERS_TD)
#undef FLATBUFFERS_TD
};
...
static void CheckBitsFit(int64_t val, size_t bits) {
    ...
    if (bits < 64 && (val & ~mask) != 0 &&  // Positive or unsigned.
        (val | mask) != -1)                 // Negative.
...
template <>
inline bool atot<bool>(const char *s) {
    return 0 != atoi(s);
}
template <>
inline float atot<float>(const char *s) {
    return static_cast<float>(strtod(s, nullptr));
}
template <>
inline double atot<double>(const char *s) {
    return strtod(s, nullptr);
}
template <>
inline Offset<void> atot<Offset<void>>(const char *s) {
    return Offset<void>(atoi(s));
}
// Declare tokens we'll use. Single character tokens are represented by their
// ascii character code (e.g. '{'), others above 256.
#define FLATBUFFERS_GEN_TOKENS(TD)                                                                                                                                                                                                                   \
    TD(Eof, 256, "end of file") TD(StringConstant, 257, "string constant") TD(IntegerConstant, 258, "integer constant") TD(FloatConstant, 259, "float constant") TD(Identifier, 260, "identifier") TD(Table, 261, "table") TD(Struct, 262, "struct") \
        TD(Enum, 263, "enum") TD(Union, 264, "union") TD(NameSpace, 265, "namespace") TD(RootType, 266, "root_type") TD(FileIdentifier, 267, "file_identifier") TD(FileExtension, 268, "file_extension") TD(Include, 269, "include")
#ifdef __GNUC__
...
#define FLATBUFFERS_TD(ENUM, IDLTYPE, CTYPE, JTYPE, GTYPE, NTYPE) kToken##ENUM,
        FLATBUFFERS_GEN_TYPES(FLATBUFFERS_TD)
#undef FLATBUFFERS_TD
    };
...
int64_t Parser::ParseHexNum(int nibbles) {
    for (int i = 0; i < nibbles; i++)
        if (!isxdigit(cursor_[i])) Error("escape code must be followed by " + NumToString(nibbles) + " hex digits");
    ...
    for (;;) {
        ...
        switch (c) {
            ...
            case '\"':
                attribute_ = "";
                while (*cursor_ != '\"') {
                    if (*cursor_ < ' ' && *cursor_ >= 0) Error("illegal character in string constant");
                    if (*cursor_ == '\\') {
                        cursor_++;
                        switch (*cursor_) {
                            case 'n':
            ...
            case '/':
                if (*cursor_ == '/') {
                    ...
                    if (*start == '/') {  // documentation comment
                        if (!seen_newline) Error("a documentation comment should be on a line on its own");
            ...
            default:
                if (isalpha(static_cast<unsigned char>(c))) {
                    ...
                    while (isalnum(static_cast<unsigned char>(*cursor_)) || *cursor_ == '_') cursor_++;
                    attribute_.clear();
...
FieldDef &Parser::AddField(StructDef &struct_def, const std::string &name, const Type &type) {
    ...
    field.value.offset = FieldIndexToOffset(static_cast<voffset_t>(struct_def.fields.vec.size()));
    field.name = name;
    ...
    if (struct_def.fields.Add(name, &field)) Error("field already exists: " + name);
    return field;
}
void Parser::ParseField(StructDef &struct_def) {
    ...
    Type type;
    ParseType(type);
    if (struct_def.fixed && !IsScalar(type.base_type) && !IsStruct(type)) Error("structs_ may contain only scalar or struct fields");
    FieldDef *typefield = nullptr;
    if (type.base_type == BASE_TYPE_UNION) {
        // For union fields, add a second auto-generated field to hold the type,
        // with _type appended as the name.
        typefield = &AddField(struct_def, name + "_type", type.enum_def->underlying_type);
    }
    auto &field = AddField(struct_def, name, type);
    if (token_ == '=') {
        Next();
        if (!IsScalar(type.base_type)) Error("default values currently only supported for scalars");
        ParseSingleValue(field.value);
    }
    if (type.enum_def && IsScalar(type.base_type) && !struct_def.fixed && !type.enum_def->attributes.Lookup("bit_flags") && !type.enum_def->ReverseLookup(static_cast<int>(StringToInt(field.value.constant.c_str()))))
        Error("enum " + type.enum_def->name + " does not have a declaration for this field\'s default of " + field.value.constant);
    field.doc_comment = dc;
    ParseMetaData(field);
    field.deprecated = field.attributes.Lookup("deprecated") != nullptr;
    if (field.deprecated && struct_def.fixed) Error("can't deprecate fields in a struct");
    field.required = field.attributes.Lookup("required") != nullptr;
    if (field.required && (struct_def.fixed || IsScalar(field.value.type.base_type))) Error("only non-scalar fields in tables may be 'required'");
    auto nested = field.attributes.Lookup("nested_flatbuffer");
    if (nested) {
        if (nested->type.base_type != BASE_TYPE_STRING) Error("nested_flatbuffer attribute must be a string (the root type)");
        if (field.value.type.base_type != BASE_TYPE_VECTOR || field.value.type.element != BASE_TYPE_UCHAR) Error("nested_flatbuffer attribute may only apply to a vector of ubyte");
...
void Parser::ParseAnyValue(Value &val, FieldDef *field) {
    switch (val.type.base_type) {
        case BASE_TYPE_UNION: {
            assert(field);
            if (!field_stack_.size() || field_stack_.back().second->value.type.base_type != BASE_TYPE_UTYPE) Error("missing type field before this union value: " + field->name);
            auto enum_idx = atot<unsigned char>(field_stack_.back().first.constant.c_str());
...
uoffset_t Parser::ParseTable(const StructDef &struct_def) {
    ...
    if (!IsNext('}'))
        for (;;) {
            ...
            if (struct_def.fixed && (fieldn >= struct_def.fields.vec.size() || struct_def.fields.vec[fieldn] != field)) {
    ...
    for (auto it = field_stack_.rbegin(); it != field_stack_.rbegin() + fieldn; ++it) {
        if (it->second->used) Error("field set more than once: " + it->second->name);
        it->second->used = true;
    }
    for (auto it = field_stack_.rbegin(); it != field_stack_.rbegin() + fieldn; ++it) {
        it->second->used = false;
    }
    if (struct_def.fixed && fieldn != struct_def.fields.vec.size()) Error("incomplete struct initialization: " + struct_def.name);
    auto start = struct_def.fixed ? builder_.StartStruct(struct_def.minalign) : builder_.StartTable();
...
#define FLATBUFFERS_TD(ENUM, IDLTYPE, CTYPE, JTYPE, GTYPE, NTYPE)                                                               \
    case BASE_TYPE_##ENUM:                                                                                                      \
        ...
        if (struct_def.fixed) {                                                                                                 \
            ...
            builder_.AddElement(value.offset, atot<CTYPE>(value.constant.c_str()), atot<CTYPE>(field->value.constant.c_str())); \
        }                                                                                                                       \
        break;
...
#define FLATBUFFERS_TD(ENUM, IDLTYPE, CTYPE, JTYPE, GTYPE, NTYPE)                  \
    case BASE_TYPE_##ENUM:                                                         \
        ...
        if (IsStruct(field->value.type)) {                                         \
            ...
            builder_.AddOffset(value.offset, atot<CTYPE>(value.constant.c_str())); \
        }                                                                          \
        break;
...
#undef FLATBUFFERS_TD
    ...
    if (struct_def.fixed) {
        ...
        struct_stack_.insert(struct_stack_.end(), builder_.GetBufferPointer(), builder_.GetBufferPointer() + struct_def.bytesize);
        builder_.PopBytes(struct_def.bytesize);
        return static_cast<uoffset_t>(off);
    } else {
        return builder_.EndTable(start, static_cast<voffset_t>(struct_def.fields.vec.size()));
    }
}
...
#define FLATBUFFERS_TD(ENUM, IDLTYPE, CTYPE, JTYPE, GTYPE, NTYPE)    \
    case BASE_TYPE_##ENUM:                                           \
        if (IsStruct(val.type))                                      \
            SerializeStruct(*val.type.struct_def, val);              \
        else                                                         \
            builder_.PushElement(atot<CTYPE>(val.constant.c_str())); \
        break;
            FLATBUFFERS_GEN_TYPES(FLATBUFFERS_TD)
#undef FLATBUFFERS_TD
        }
    ...
    if (match) {
        ...
        if (!check) {
            if (e.type.base_type == BASE_TYPE_NONE) {
                ...
                Error(std::string("type mismatch: expecting: ") + kTypeNames[e.type.base_type] + ", found: " + kTypeNames[req]);
            }
        }
        Next();
    }
...
int64_t Parser::ParseIntegerFromString(Type &type) {
    ...
    do {
        ...
        if (type.enum_def) {  // The field has an enum type
            ...
            if (!enum_val) Error("unknown enum value: " + word + ", for enum: " + type.enum_def->name);
            result |= enum_val->value;
        } else {  // No enum type, probably integral field.
            if (!IsInteger(type.base_type)) Error("not a valid value for this field: " + word);
...
void Parser::ParseSingleValue(Value &e) {
    ...
    if (e.type.base_type != BASE_TYPE_STRING && e.type.base_type != BASE_TYPE_NONE && (token_ == kTokenIdentifier || token_ == kTokenStringConstant)) {
        e.constant = NumToString(ParseIntegerFromString(e.type));
        Next();
    } else if (TryTypedValue(kTokenIntegerConstant, IsScalar(e.type.base_type), e, BASE_TYPE_INT) || TryTypedValue(kTokenFloatConstant, IsFloat(e.type.base_type), e, BASE_TYPE_FLOAT) ||
               TryTypedValue(kTokenStringConstant, e.type.base_type == BASE_TYPE_STRING, e, BASE_TYPE_STRING)) {
    } else {
...
void Parser::ParseEnum(bool is_union) {
    ...
    if (is_union) {
        ...
        if (proto_mode_) {
            ...
            if (!IsInteger(enum_def.underlying_type.base_type)) Error("underlying enum type must be integral");
        }
    ...
    do {
        ...
        auto value = enum_def.vals.vec.size() ? enum_def.vals.vec.back()->value + 1 : 0;
        auto &ev = *new EnumVal(name, value);
        if (enum_def.vals.Add(name, &ev)) Error("enum value already exists: " + name);
        ev.doc_comment = dc;
        if (is_union) {
        ...
        if (IsNext('=')) {
            ...
            if (prevsize && enum_def.vals.vec[prevsize - 1]->value >= ev.value) Error("enum values must be specified in ascending order");
        }
    } while (IsNext(proto_mode_ ? ';' : ',') && token_ != '}');
    Expect('}');
    if (enum_def.attributes.Lookup("bit_flags")) {
        for (auto it = enum_def.vals.vec.begin(); it != enum_def.vals.vec.end(); ++it) {
            if (static_cast<size_t>((*it)->value) >= SizeOf(enum_def.underlying_type.base_type) * 8) Error("bit flag out of range of underlying integral type");
...
void Parser::ParseDecl() {
    ...
    struct_def.sortbysize = struct_def.attributes.Lookup("original_order") == nullptr && !fixed;
    Expect('{');
    while (token_ != '}') ParseField(struct_def);
    auto force_align = struct_def.attributes.Lookup("force_align");
    if (fixed && force_align) {
        auto align = static_cast<size_t>(atoi(force_align->constant.c_str()));
        if (force_align->type.base_type != BASE_TYPE_INT || align < struct_def.minalign || align > 256 || align & (align - 1))
            Error(
    ...
    if (!struct_def.fixed && fields.size()) {
        ...
        if (num_id_fields) {
            ...
            if (num_id_fields != fields.size()) Error("either all fields or no fields must have an 'id' attribute");
            // Simply sort by id, then the fields are the same as if no ids had
            // been specified.
            std::sort(fields.begin(), fields.end(), [](const FieldDef * a, const FieldDef * b)->bool {
            ...
            for (int i = 0; i < static_cast<int>(fields.size()); i++) {
                if (i != atoi(fields[i]->attributes.Lookup("id")->constant.c_str())) Error("field id\'s must be consecutive from 0, id " + NumToString(i) + " missing or set twice");
    ...
    auto CheckClash = [&fields, &struct_def](const char *suffix, BaseType basetype) {
        auto len = strlen(suffix);
        for (auto it = fields.begin(); it != fields.end(); ++it) {
            auto &name = (*it)->name;
            if (name.length() > len && name.compare(name.length() - len, len, suffix) == 0 && (*it)->value.type.base_type != BASE_TYPE_UTYPE) {
                auto field = struct_def.fields.Lookup(name.substr(0, name.length() - len));
                if (field && field->value.type.base_type == basetype) Error("Field " + name + " would clash with generated functions for field " + field->name);
            }
        }
    };
...
void Parser::ParseProtoDecl() {
    if (attribute_ == "package") {
        ...
        Error("don\'t know how to parse .proto declaration starting with " + attribute_);
    }
}
...
Type Parser::ParseTypeFromProtoType() {
    ...
    static type_lookup lookup[] = {{"float", BASE_TYPE_FLOAT},
                                   {"double", BASE_TYPE_DOUBLE},
                                   {"int32", BASE_TYPE_INT},
                                   {"int64", BASE_TYPE_LONG},
                                   {"uint32", BASE_TYPE_UINT},
                                   {"uint64", BASE_TYPE_ULONG},
                                   {"sint32", BASE_TYPE_INT},
                                   {"sint64", BASE_TYPE_LONG},
                                   {"fixed32", BASE_TYPE_UINT},
                                   {"fixed64", BASE_TYPE_ULONG},
                                   {"sfixed32", BASE_TYPE_INT},
                                   {"sfixed64", BASE_TYPE_LONG},
                                   {"bool", BASE_TYPE_BOOL},
...
bool Parser::Parse(const char *source, const char **include_paths, const char *source_filename) {
    ...
    try {
        ...
        while (IsNext(kTokenInclude)) {
            ...
            if (included_files_.find(name) == included_files_.end()) {
                ...
                if (contents.empty()) Error("unable to load include file: " + name);
        ...
        while (token_ != kTokenEof) {
            if (proto_mode_) {
                ...
                builder_.Finish(Offset<Table>(ParseTable(*root_struct_def)), file_identifier_.length() ? file_identifier_.c_str() : nullptr);
                ...
                Expect(kTokenIdentifier);
                if (!SetRootType(root_type.c_str())) Error("unknown root type: " + root_type);
                if (root_struct_def->fixed) Error("root type must be a table");
                Expect(';');
            } else if (token_ == kTokenFileIdentifier) {
                Next();
                file_identifier_ = attribute_;
                Expect(kTokenStringConstant);
                if (file_identifier_.length() != FlatBufferBuilder::kFileIdentifierLength) Error("file_identifier must be exactly " + NumToString(FlatBufferBuilder::kFileIdentifierLength) + " characters");
                Expect(';');
        ...
        for (auto it = structs_.vec.begin(); it != structs_.vec.end(); ++it) {
            if ((*it)->predecl) Error("type referenced but not defined: " + (*it)->name);
        }
        for (auto it = enums_.vec.begin(); it != enums_.vec.end(); ++it) {
            auto &enum_def = **it;
            if (enum_def.is_union) {
                for (auto it = enum_def.vals.vec.begin(); it != enum_def.vals.vec.end(); ++it) {
                    auto &val = **it;
                    if (val.struct_def && val.struct_def->fixed) Error("only tables can be union elements: " + val.name);
                }
            }
        }
    }
