...
#include <vector>
#include <algorithm>
#if __cplusplus <= 199711L && (!defined(_MSC_VER) || _MSC_VER < 1600) && (!defined(__GNUC__) || (__GNUC__ * 10000 + __GNUC_MINOR__ * 100 + __GNUC_PATCHLEVEL__ < 40603))
...
struct VectorIterator : public std::iterator<std::input_iterator_tag, typename std::conditional<bConst, const typename IndirectHelper<T>::return_type, typename IndirectHelper<T>::return_type>::type, uoffset_t> {
    typedef std::iterator<std::input_iterator_tag, typename std::conditional<bConst, const typename IndirectHelper<T>::return_type, typename IndirectHelper<T>::return_type>::type, uoffset_t> super_type;
   public:
...
class vector_downward {
   public:
    explicit vector_downward(size_t initial_size, const simple_allocator &allocator) : reserved_(initial_size), buf_(allocator.allocate(reserved_)), cur_(buf_ + reserved_), allocator_(allocator) {
        assert((initial_size & (sizeof(largest_scalar_t) - 1)) == 0);
    }
    ~vector_downward() { allocator_.deallocate(buf_); }
    void clear() { cur_ = buf_ + reserved_; }
    size_t growth_policy(size_t bytes) { return (bytes / 2) & ~(sizeof(largest_scalar_t) - 1); }
...
class FlatBufferBuilder {
   public:
    explicit FlatBufferBuilder(uoffset_t initial_size = 1024, const simple_allocator *allocator = nullptr) : buf_(initial_size, allocator ? *allocator : default_allocator), minalign_(1), force_defaults_(false) {
    ...
    uoffset_t EndVector(size_t len) { return PushElement(static_cast<uoffset_t>(len)); }
    ...
    template <typename T>
    Offset<Vector<T>> CreateUninitializedVector(size_t len) {
        NotNested();
        StartVector(len, sizeof(T));
    ...
    void Finish(Offset<T> root, const char *file_identifier = nullptr) {
        ...
        PreAlign(sizeof(uoffset_t) + (file_identifier ? kFileIdentifierLength : 0), minalign_);
        if (file_identifier) {
...
const T *GetRoot(const void *buf) {
    EndianCheck();
    return reinterpret_cast<const T *>(reinterpret_cast<const uint8_t *>(buf) + EndianScalar(*reinterpret_cast<const uoffset_t *>(buf)));
}
// Helper to see if the identifier in a buffer has the expected value.
inline bool BufferHasIdentifier(const void *buf, const char *identifier) { return strncmp(reinterpret_cast<const char *>(buf) + sizeof(uoffset_t), identifier, FlatBufferBuilder::kFileIdentifierLength) == 0; }
// Helper class to verify the integrity of a FlatBuffer
class Verifier {
   public:
    Verifier(const uint8_t *buf, size_t buf_len, size_t _max_depth = 64, size_t _max_tables = 1000000) : buf_(buf), end_(buf + buf_len), depth_(0), max_depth_(_max_depth), num_tables_(0), max_tables_(_max_tables) {}
    ...
    bool Verify(const void *elem, size_t elem_len) const { return Check(elem >= buf_ && elem <= end_ - elem_len); }
    ...
    bool VerifyBuffer() {
        ...
        return Verify<uoffset_t>(buf_) && reinterpret_cast<const T *>(buf_ + ReadScalar<uoffset_t>(buf_))->Verify(*this);
    }
...
class Table {
   public:
    ...
    bool VerifyTableStart(Verifier &verifier) const {
        ...
        return verifier.VerifyComplexity() && verifier.Verify<voffset_t>(vtable) && verifier.Verify(vtable, ReadScalar<voffset_t>(vtable));
    }
