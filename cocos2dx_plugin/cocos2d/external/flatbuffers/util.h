...
inline bool SaveFile(const char *name, const std::string &buf, bool binary) { return SaveFile(name, buf.c_str(), buf.size(), binary); }
...
inline std::string ConCatPathFileName(const std::string &path, const std::string &filename) {
    ...
    if (path.length() && path.back() != kPathSeparator && path.back() != kPosixPathSeparator) filepath += kPathSeparator;
    filepath += filename;
    return filepath;
}
