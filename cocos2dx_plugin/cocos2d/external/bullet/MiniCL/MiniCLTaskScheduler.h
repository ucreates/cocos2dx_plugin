...
class MiniCLTaskScheduler {
   ...
   public:
    ...
    void flush();
    class btThreadSupportInterface* getThreadSupportInterface() { return m_threadInterface; }
    int findProgramCommandIdByName(const char* programName) const;
    int getMaxNumOutstandingTasks() const { return m_maxNumOutstandingTasks; }
    void registerKernel(MiniCLKernel* kernel) { m_kernels.push_back(kernel); }
};
