...
btMLCPSolver::btMLCPSolver(btMLCPSolverInterface* solver) : m_solver(solver), m_fallback(0) {}
btMLCPSolver::~btMLCPSolver() {}
bool gUseMatrixMultiply = false;
bool interleaveContactAndFriction = false;
btScalar btMLCPSolver::solveGroupCacheFriendlySetup(btCollisionObject** bodies, int numBodiesUnUsed, btPersistentManifold** manifoldPtr, int numManifolds, btTypedConstraint** constraints, int numConstraints, const btContactSolverInfo& infoGlobal,
...
bool btMLCPSolver::solveMLCP(const btContactSolverInfo& infoGlobal) {
    ...
    if (infoGlobal.m_splitImpulse) {
        ...
        if (result) result = m_solver->solveMLCP(Acopy, m_bSplit, m_xSplit, m_lo, m_hi, limitDependenciesCopy, infoGlobal.m_numIterations);
    } else {
        result = m_solver->solveMLCP(m_A, m_b, m_x, m_lo, m_hi, m_limitDependencies, infoGlobal.m_numIterations);
    }
    return result;
            ...
            const btScalar* JinvMrow = JinvM + 2 * 8 * (size_t)row__;
            {
                ...
                while (startJointNodeA >= 0) {
                    ...
                    if (j0 < c) {
                        ...
                        m_A.multiplyAdd2_p8r(JinvMrow, Jptr + 2 * 8 * (size_t)ofs[j0] + ofsother, numRows, numRowsOther, row__, ofs[j0]);
                    }
            ...
            }
            {
                ...
                while (startJointNodeB >= 0) {
                    ...
                    if (j1 < c) {
                        ...
                        m_A.multiplyAdd2_p8r(JinvMrow + 8 * (size_t)numRows, Jptr + 2 * 8 * (size_t)ofs[j1] + ofsother, numRows, numRowsOther, row__, ofs[j1]);
                    }
...
void btMLCPSolver::createMLCP(const btContactSolverInfo& infoGlobal) {
    ...
    for (int i = 0; i < numConstraintRows; i++) {
        if (m_allConstraintArray[i].m_jacDiagABInv) {
            m_b[i] = m_allConstraintArray[i].m_rhs / m_allConstraintArray[i].m_jacDiagABInv;
            if (infoGlobal.m_splitImpulse) m_bSplit[i] = m_allConstraintArray[i].m_rhsPenetration / m_allConstraintArray[i].m_jacDiagABInv;
        }
    }
    ...
    for (int i = 0; i < numBodies; i++) {
        ...
        for (int r = 0; r < 3; r++)
            for (int c = 0; c < 3; c++) setElem(Minv, i * 6 + 3 + r, i * 6 + 3 + c, orgBody ? orgBody->getInvInertiaTensorWorld()[r][c] : 0);
    }
    static btMatrixXu J;
...
btScalar btMLCPSolver::solveGroupCacheFriendlyIterations(btCollisionObject** bodies, int numBodies, btPersistentManifold** manifoldPtr, int numManifolds, btTypedConstraint** constraints, int numConstraints, const btContactSolverInfo& infoGlobal,
                                                         btIDebugDraw* debugDrawer) {
    bool result = true;
    {
