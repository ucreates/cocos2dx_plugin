...
void btConeTwistConstraint::getInfo2(btConstraintInfo2* info) { getInfo2NonVirtual(info, m_rbA.getCenterOfMassTransform(), m_rbB.getCenterOfMassTransform(), m_rbA.getInvInertiaTensorWorld(), m_rbB.getInvInertiaTensorWorld()); }
    ...
    if (m_useSolveConstraintObsolete) {
        ...
        if (m_bMotorEnabled) {
            ...
            if (bDoTorque && avgAxis.length2() > SIMD_EPSILON) {
                ...
                btVector3 impulse = (kAxisAInv * dOmegaA - kAxisBInv * dOmegaB) / (kInvCombined * kInvCombined);
        ...
        } else if (m_damping > SIMD_EPSILON)  // no motor: do a little damping
        {
            ...
            if (relVel.length2() > SIMD_EPSILON) {
                ...
                btScalar m_kDamping = btScalar(1.) / (getRigidBodyA().computeAngularImpulseDenominator(relVelAxis) + getRigidBodyB().computeAngularImpulseDenominator(relVelAxis));
        ...
        // joint limits
        {
            ...
            if (m_solveSwingLimit) {
                ...
                if (relSwingVel > 0) amplitude += m_swingLimitRatio * relSwingVel * m_relaxationFactor;
    ...
    }
    {
        ...
        if (m_swingSpan1 >= m_fixThresh && m_swingSpan2 >= m_fixThresh) {
            ...
            if (swingAngle > swingLimit * m_limitSoftness) {
                ...
                m_kSwing = btScalar(1.) / (computeAngularImpulseDenominator(m_swingAxis, invInertiaWorldA) + computeAngularImpulseDenominator(m_swingAxis, invInertiaWorldB));
            }
        } else {
        ...
        if (m_twistSpan >= btScalar(0.f)) {
            ...
            if (m_twistAngle > m_twistSpan * m_limitSoftness) {
                ...
                m_kTwist = btScalar(1.) / (computeAngularImpulseDenominator(m_twistAxis, invInertiaWorldA) + computeAngularImpulseDenominator(m_twistAxis, invInertiaWorldB));
            }
...
void btConeTwistConstraint::computeConeLimitInfo(const btQuaternion& qCone, btScalar& swingAngle,  // out
...
void btConeTwistConstraint::computeTwistLimitInfo(const btQuaternion& qTwist, btScalar& twistAngle,  // out
