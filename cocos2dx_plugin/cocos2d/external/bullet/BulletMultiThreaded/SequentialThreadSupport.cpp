...
SequentialThreadSupport::SequentialThreadSupport(SequentialThreadConstructionInfo& threadConstructionInfo) { startThreads(threadConstructionInfo); }
/// cleanup/shutdown Libspe2
SequentialThreadSupport::~SequentialThreadSupport() { stopSPU(); }
#include <stdio.h>
...
void SequentialThreadSupport::stopSPU() { m_activeSpuStatus.clear(); }
void SequentialThreadSupport::setNumTasks(int numTasks) { printf("SequentialThreadSupport::setNumTasks(%d) is not implemented and has no effect\n", numTasks); }
...
class btDummyCriticalSection : public btCriticalSection
   ...
   public:
    ...
    void lock() { mCommonBuff[0] = 1; }
    void unlock() { mCommonBuff[0] = 0; }
};
btBarrier* SequentialThreadSupport::createBarrier() { return new btDummyBarrier(); }
btCriticalSection* SequentialThreadSupport::createCriticalSection() { return new btDummyCriticalSection(); }
void SequentialThreadSupport::deleteBarrier(btBarrier* barrier) { delete barrier; }
void SequentialThreadSupport::deleteCriticalSection(btCriticalSection* criticalSection) { delete criticalSection; }
