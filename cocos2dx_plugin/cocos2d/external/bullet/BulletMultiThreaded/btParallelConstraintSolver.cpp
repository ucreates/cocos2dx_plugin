...
static SIMD_FORCE_INLINE void pfxSolveLinearConstraintRow(btConstraintRow& constraint, vmVector3& deltaLinearVelocityA, vmVector3& deltaAngularVelocityA, float massInvA, const vmMatrix3& inertiaInvA, const vmVector3& rA,
                                                          vmVector3& deltaLinearVelocityB, vmVector3& deltaAngularVelocityB, float massInvB, const vmMatrix3& inertiaInvB, const vmVector3& rB) {
...
void btSolveContactConstraint(btConstraintRow& constraintResponse, btConstraintRow& constraintFriction1, btConstraintRow& constraintFriction2, const vmVector3& contactPointA, const vmVector3& contactPointB, PfxSolverBody& solverBodyA,
                              PfxSolverBody& solverBodyB, float friction) {
    vmVector3 rA = rotate(solverBodyA.mOrientation, contactPointA);
    vmVector3 rB = rotate(solverBodyB.mOrientation, contactPointB);
    pfxSolveLinearConstraintRow(constraintResponse, solverBodyA.mDeltaLinearVelocity, solverBodyA.mDeltaAngularVelocity, solverBodyA.mMassInv, solverBodyA.mInertiaInv, rA, solverBodyB.mDeltaLinearVelocity, solverBodyB.mDeltaAngularVelocity,
    ...
    pfxSolveLinearConstraintRow(constraintFriction1, solverBodyA.mDeltaLinearVelocity, solverBodyA.mDeltaAngularVelocity, solverBodyA.mMassInv, solverBodyA.mInertiaInv, rA, solverBodyB.mDeltaLinearVelocity, solverBodyB.mDeltaAngularVelocity,
                                solverBodyB.mMassInv, solverBodyB.mInertiaInv, rB);
    pfxSolveLinearConstraintRow(constraintFriction2, solverBodyA.mDeltaLinearVelocity, solverBodyA.mDeltaAngularVelocity, solverBodyA.mMassInv, solverBodyA.mInertiaInv, rA, solverBodyB.mDeltaLinearVelocity, solverBodyB.mDeltaAngularVelocity,
                                solverBodyB.mMassInv, solverBodyB.mInertiaInv, rB);
}
void CustomSolveConstraintsTaskParallel(const PfxParallelGroup* contactParallelGroup, const PfxParallelBatch* contactParallelBatches, PfxConstraintPair* contactPairs, uint32_t numContactPairs, btPersistentManifold* offsetContactManifolds,
                                        btConstraintRow* offsetContactConstraintRows, const PfxParallelGroup* jointParallelGroup, const PfxParallelBatch* jointParallelBatches, PfxConstraintPair* jointPairs, uint32_t numJointPairs,
                                        btSolverConstraint* offsetSolverConstraints, TrbState* offsetRigStates, PfxSolverBody* offsetSolverBodies, uint32_t numRigidBodies, int iteration, unsigned int taskId, unsigned int numTasks,
                                        btBarrier* barrier) {
                    ...
                    for (uint32_t i = 0; i < numPairs; i++) {
                        ...
                        for (int j = 0; j < contact.getNumContacts(); j++) {
                            ...
                            if (k == 0) {
                                ...
                                float imp[3] = {cp.m_appliedImpulse, cp.m_appliedImpulseLateral1, cp.m_appliedImpulseLateral2};
                                for (int k = 0; k < 3; k++) {
                                ...
                                btSolveContactConstraint(contactConstraintRows[j * 3], contactConstraintRows[j * 3 + 1], contactConstraintRows[j * 3 + 2], btReadVector3(cp.m_localPointA), btReadVector3(cp.m_localPointB), solverBodyA, solverBodyB,
...
static SIMD_FORCE_INLINE void pfxGetPlaneSpace(const vmVector3& n, vmVector3& p, vmVector3& q) {
    if (fabsf(n[2]) > 0.707f) {
...
void btSetupContactConstraint(btConstraintRow& constraintResponse, btConstraintRow& constraintFriction1, btConstraintRow& constraintFriction2, float penetrationDepth, float restitution, float friction, const vmVector3& contactNormal,
                              const vmVector3& contactPointA, const vmVector3& contactPointB, const TrbState& stateA, const TrbState& stateB, PfxSolverBody& solverBodyA, PfxSolverBody& solverBodyB, const vmVector3& linVelA, const vmVector3& angVelA,
                              const vmVector3& linVelB, const vmVector3& angVelB, float separateBias, float timeStep) {
    vmVector3 rA = rotate(solverBodyA.mOrientation, contactPointA);
    vmVector3 rB = rotate(solverBodyB.mOrientation, contactPointB);
    vmMatrix3 K = vmMatrix3::scale(vmVector3(solverBodyA.mMassInv + solverBodyB.mMassInv)) - crossMatrix(rA) * solverBodyA.mInertiaInv * crossMatrix(rA) - crossMatrix(rB) * solverBodyB.mInertiaInv * crossMatrix(rB);
...
void CustomSetupContactConstraintsTask(PfxConstraintPair* contactPairs, uint32_t numContactPairs, btPersistentManifold* offsetContactManifolds, btConstraintRow* offsetContactConstraintRows, TrbState* offsetRigStates,
                                       PfxSolverBody* offsetSolverBodies, uint32_t numRigidBodies, float separateBias, float timeStep) {
    for (uint32_t i = 0; i < numContactPairs; i++) {
        PfxConstraintPair& pair = contactPairs[i];
        if (!pfxGetActive(pair) || pfxGetNumConstraints(pair) == 0 || ((pfxGetMotionMaskA(pair) & PFX_MOTION_MASK_STATIC) && (pfxGetMotionMaskB(pair) & PFX_MOTION_MASK_STATIC))) {
            continue;
        }
        uint16_t iA = pfxGetRigidBodyIdA(pair);
        uint16_t iB = pfxGetRigidBodyIdB(pair);
        ...
        for (int j = 0; j < contact.getNumContacts(); j++) {
            ...
            btSetupContactConstraint(contactConstraintRows[j * 3], contactConstraintRows[j * 3 + 1], contactConstraintRows[j * 3 + 2], cp.getDistance(), restitution, friction, btReadVector3(cp.m_normalWorldOnB),  //.mConstraintRow[0].m_normal),
                                     btReadVector3(cp.m_localPointA), btReadVector3(cp.m_localPointB), stateA, stateB, solverBodyA, solverBodyB, (const vmVector3&)linVelA, (const vmVector3&)angVelA, (const vmVector3&)linVelB,
                                     (const vmVector3&)angVelB, separateBias, timeStep);
        }
        // contact.setCompositeFriction(friction);
    }
}
void CustomWritebackContactConstraintsTask(PfxConstraintPair* contactPairs, uint32_t numContactPairs, btPersistentManifold* offsetContactManifolds, btConstraintRow* offsetContactConstraintRows, TrbState* offsetRigStates,
                                           PfxSolverBody* offsetSolverBodies, uint32_t numRigidBodies, float separateBias, float timeStep) {
    for (uint32_t i = 0; i < numContactPairs; i++) {
        PfxConstraintPair& pair = contactPairs[i];
        if (!pfxGetActive(pair) || pfxGetNumConstraints(pair) == 0 || ((pfxGetMotionMaskA(pair) & PFX_MOTION_MASK_STATIC) && (pfxGetMotionMaskB(pair) & PFX_MOTION_MASK_STATIC))) {
...
void SolverThreadFunc(void* userPtr, void* lsMemory) {
    ...
    switch (io->cmd) {
        case PFX_CONSTRAINT_SOLVER_CMD_SOLVE_CONSTRAINTS:
            CustomSolveConstraintsTaskParallel(io->solveConstraints.contactParallelGroup, io->solveConstraints.contactParallelBatches, io->solveConstraints.contactPairs, io->solveConstraints.numContactPairs,
                                               io->solveConstraints.offsetContactManifolds, io->solveConstraints.offsetContactConstraintRows, io->solveConstraints.jointParallelGroup, io->solveConstraints.jointParallelBatches,
                                               io->solveConstraints.jointPairs, io->solveConstraints.numJointPairs, io->solveConstraints.offsetSolverConstraints, io->solveConstraints.offsetRigStates1, io->solveConstraints.offsetSolverBodies,
                                               io->solveConstraints.numRigidBodies, io->solveConstraints.iteration, io->solveConstraints.taskId, io->maxTasks1, io->solveConstraints.barrier);
            break;
        ...
        case PFX_CONSTRAINT_SOLVER_CMD_SETUP_CONTACT_CONSTRAINTS: {
            ...
            while (!empty) {
                ...
                if (batch > 0) {
                    CustomSetupContactConstraintsTask(io->setupContactConstraints.offsetContactPairs + start, batch, io->setupContactConstraints.offsetContactManifolds, io->setupContactConstraints.offsetContactConstraintRows,
                                                      io->setupContactConstraints.offsetRigStates,
                                                      //                        io->setupContactConstraints.offsetRigBodies,
                                                      io->setupContactConstraints.offsetSolverBodies, io->setupContactConstraints.numRigidBodies, io->setupContactConstraints.separateBias, io->setupContactConstraints.timeStep);
                } else {
                    empty = true;
                }
            }
        ...
        case PFX_CONSTRAINT_SOLVER_CMD_WRITEBACK_APPLIED_IMPULSES_CONTACT_CONSTRAINTS: {
            ...
            while (!empty) {
                ...
                criticalsection->unlock();
                if (batch > 0) {
                    CustomWritebackContactConstraintsTask(io->setupContactConstraints.offsetContactPairs + start, batch, io->setupContactConstraints.offsetContactManifolds, io->setupContactConstraints.offsetContactConstraintRows,
                                                          io->setupContactConstraints.offsetRigStates,
                                                          //                        io->setupContactConstraints.offsetRigBodies,
                                                          io->setupContactConstraints.offsetSolverBodies, io->setupContactConstraints.numRigidBodies, io->setupContactConstraints.separateBias, io->setupContactConstraints.timeStep);
                } else {
...
void CustomSetupContactConstraintsNew(PfxConstraintPair* contactPairs1, uint32_t numContactPairs, btPersistentManifold* offsetContactManifolds, btConstraintRow* offsetContactConstraintRows, TrbState* offsetRigStates,
                                      PfxSolverBody* offsetSolverBodies, uint32_t numRigidBodies, float separationBias, float timeStep, class btThreadSupportInterface* threadSupport, btCriticalSection* criticalSection, btConstraintSolverIO* io,
                                      uint8_t cmd) {
    ...
    for (phaseId = 0; phaseId < PFX_MAX_SOLVER_PHASES && totalCount < numPairs; phaseId++) {
        ...
        for (batchId = 0; i < numPairs && totalCount < numPairs && batchId < maxBatches; batchId++) {
            ...
            for (; i < numPairs && pairCount < targetCount; i++) {
                ...
                if (!pfxGetActive(pairs[i]) || pfxGetNumConstraints(pairs[i]) == 0 || ((pfxGetMotionMaskA(pairs[i]) & PFX_MOTION_MASK_STATIC) && (pfxGetMotionMaskB(pairs[i]) & PFX_MOTION_MASK_STATIC))) {
                ...
                // 依存性のチェック
                if ((bodyTable[idxA] != batchId && bodyTable[idxA] != 0xff) || (bodyTable[idxB] != batchId && bodyTable[idxB] != 0xff)) {
                    startIndexCheck = false;
                    // bodies of the pair are already assigned to another batch within this phase
                    continue;
                }
                // 依存性判定テーブルに登録
                if (pfxGetMotionMaskA(pairs[i]) & PFX_MOTION_MASK_DYNAMIC) bodyTable[idxA] = batchId;
                if (pfxGetMotionMaskB(pairs[i]) & PFX_MOTION_MASK_DYNAMIC) bodyTable[idxB] = batchId;
                if (startIndexCheck) startIndex++;
                pairTable[idxP] |= maskP;
...
void CustomSolveConstraintsParallel(PfxConstraintPair* contactPairs, uint32_t numContactPairs, PfxConstraintPair* jointPairs, uint32_t numJointPairs, btPersistentManifold* offsetContactManifolds, btConstraintRow* offsetContactConstraintRows,
                                    btSolverConstraint* offsetSolverConstraints, TrbState* offsetRigStates, PfxSolverBody* offsetSolverBodies, uint32_t numRigidBodies, struct btConstraintSolverIO* io, class btThreadSupportInterface* threadSupport,
                                    int iteration, void* poolBuf, int poolBytes, class btBarrier* barrier) {
    int maxTasks = threadSupport->getNumTasks();
...
//#define SOLVE_SEQUENTIAL
            ...
            CustomSolveConstraintsTask(io->solveConstraints.contactParallelGroup, io->solveConstraints.contactParallelBatches, io->solveConstraints.contactPairs, io->solveConstraints.numContactPairs, io->solveConstraints.offsetContactManifolds,
                                       io->solveConstraints.jointParallelGroup, io->solveConstraints.jointParallelBatches, io->solveConstraints.jointPairs, io->solveConstraints.numJointPairs, io->solveConstraints.offsetSolverConstraints,
                                       io->solveConstraints.offsetRigStates1, io->solveConstraints.offsetSolverBodies, io->solveConstraints.numRigidBodies, io->solveConstraints.iteration, 0, 1, 0);  // arg->taskId,1,0);//,arg->maxTasks,arg->barrier);
#else
            for (int t = 0; t < maxTasks; t++) {
...
void BPE_customConstraintSolverSequentialNew(unsigned int new_num, PfxBroadphasePair* new_pairs1, btPersistentManifold* offsetContactManifolds, PfxConstraintRow* offsetContactConstraintRows, TrbState* states, int numRigidBodies,
                                             struct PfxSolverBody* solverBodies, PfxConstraintPair* jointPairs, unsigned int numJoints, btSolverConstraint* offsetSolverConstraints, float separateBias, float timeStep, int iteration,
                                             btThreadSupportInterface* solverThreadSupport, btCriticalSection* criticalSection, struct btConstraintSolverIO* solverIO, btBarrier* barrier) {
    {
...
#ifdef SEQUENTIAL_SETUP
        CustomSetupContactConstraintsSeqNew((PfxConstraintPair*)new_pairs1, new_num, contacts, states, solverBodies, numRigidBodies, separateBias, timeStep);
#else
        CustomSetupContactConstraintsNew((PfxConstraintPair*)new_pairs1, new_num, offsetContactManifolds, offsetContactConstraintRows, states, solverBodies, numRigidBodies, separateBias, timeStep, solverThreadSupport, criticalSection, solverIO,
...
//#define SEQUENTIAL
#ifdef SEQUENTIAL
        CustomSolveConstraintsSeq((PfxConstraintPair*)new_pairs1, new_num, contacts, jointPairs, numJoints, states, solverBodies, numRigidBodies, separateBias, timeStep, iteration);
#else  // SEQUENTIAL
        CustomSolveConstraintsParallel((PfxConstraintPair*)new_pairs1, new_num, jointPairs, numJoints, offsetContactManifolds, offsetContactConstraintRows, offsetSolverConstraints, states, solverBodies, numRigidBodies, solverIO, solverThreadSupport,
                                       iteration, tmp_buff, TMP_BUFF_BYTES, barrier);
#endif  // SEQUENTIAL
    }
    {
...
btConstraintSolverIO* createSolverIO(int numThreads) { return new btConstraintSolverIO[numThreads]; }
...
btScalar btParallelConstraintSolver::solveGroup(btCollisionObject** bodies1, int numRigidBodies, btPersistentManifold** manifoldPtr, int numManifolds, btTypedConstraint** constraints, int numConstraints, const btContactSolverInfo& infoGlobal,
    ...
    */
    ...
    int actualNumManifolds = 0;
    {
        ...
        for (int i1 = 0; i1 < numManifolds; i1++) {
            if (manifoldPtr[i1]->getNumContacts() > 0) {
                ...
                if (!obAisActive && !obBisActive) continue;
    ...
    {
        {
            ...
            for (i = 0; i < numConstraints; i++) {
                ...
                if (info1.m_numConstraintRows) {
                    ...
                    pfxSetRigidBodyIdB(pair, idB);
                    // is this needed?
                    if (idA >= 0) pfxSetMotionMaskA(pair, m_memoryCache->m_mystates[idA].getMotionMask());
                    if (idB >= 0) pfxSetMotionMaskB(pair, m_memoryCache->m_mystates[idB].getMotionMask());
    ...
    //    printf("actualNumManifolds=%d\n",actualNumManifolds);
    {
        ...
        if (numRigidBodies > 0 && (actualNumManifolds + actualNumJoints) > 0) {
            ...
            BPE_customConstraintSolverSequentialNew(actualNumManifolds, actualPairs, offsetContactManifolds, contactRows, &m_memoryCache->m_mystates[0], numRigidBodies, &m_memoryCache->m_mysolverbodies[0], jointPairs, actualNumJoints,
                                                    offsetSolverConstraints, separateBias, timeStep, iteration, m_solverThreadSupport, m_criticalSection, m_solverIO, m_barrier);
        }
    }
