...
inline TrbState::TrbState(const uint8_t m, const vmVector3& x, const vmQuat& q, const vmVector3& v, const vmVector3& omega) {
    setMotionType(m);
    fX[0] = x[0];
    fX[1] = x[1];
    fX[2] = x[2];
...
inline void TrbState::setAuxils(const vmVector3& centerLocal, const vmVector3& halfLocal) {
...
inline void TrbState::setAuxilsCcd(const vmVector3& centerLocal, const vmVector3& halfLocal, float timeStep) {
    vmVector3 centerW = getPosition() + rotate(getOrientation(), centerLocal);
    vmVector3 halfW = absPerElem(vmMatrix3(getOrientation())) * halfLocal;
