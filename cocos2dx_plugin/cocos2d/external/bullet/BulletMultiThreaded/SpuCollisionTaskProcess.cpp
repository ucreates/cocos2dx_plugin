...
SpuCollisionTaskProcess::SpuCollisionTaskProcess(class btThreadSupportInterface* threadInterface, unsigned int maxNumOutstandingTasks) : m_threadInterface(threadInterface), m_maxNumOutstandingTasks(0) {
...
void SpuCollisionTaskProcess::addWorkToTask(void* pairArrayPtr, int startIndex, int endIndex) {
    ...
    {
        SpuGatherAndProcessWorkUnitInput& wuInput = *(reinterpret_cast<SpuGatherAndProcessWorkUnitInput*>(MIDPHASE_ENTRY_PTR(m_currentTask, m_currentPage, m_currentPageEntry)));
