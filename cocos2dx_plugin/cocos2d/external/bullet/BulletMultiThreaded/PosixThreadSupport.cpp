...
PosixThreadSupport::PosixThreadSupport(ThreadConstructionInfo& threadConstructionInfo) { startThreads(threadConstructionInfo); }
// cleanup/shutdown Libspe2
PosixThreadSupport::~PosixThreadSupport() { stopSPU(); }
...
class PosixCriticalSection : public btCriticalSection {
   ...
   public:
    PosixCriticalSection() { pthread_mutex_init(&m_mutex, NULL); }
    virtual ~PosixCriticalSection() { pthread_mutex_destroy(&m_mutex); }
    ATTRIBUTE_ALIGNED16(unsigned int mCommonBuff[32]);
    virtual unsigned int getSharedParam(int i) { return mCommonBuff[i]; }
    virtual void setSharedParam(int i, unsigned int p) { mCommonBuff[i] = p; }
...
class PosixBarrier : public btBarrier {
    ...
    int m_numThreads;
   public:
    PosixBarrier() : m_numThreads(0) {}
    virtual ~PosixBarrier() { pthread_barrier_destroy(&m_barr); }
    virtual void sync() {
...
btCriticalSection* PosixThreadSupport::createCriticalSection() { return new PosixCriticalSection(); }
void PosixThreadSupport::deleteBarrier(btBarrier* barrier) { delete barrier; }
void PosixThreadSupport::deleteCriticalSection(btCriticalSection* cs) { delete cs; }
#endif  // USE_PTHREADS
