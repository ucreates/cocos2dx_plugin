...
ATTRIBUTE_ALIGNED16(struct) bvhMeshShape_LocalStoreMemory {
    ...
    btOptimizedBvh* getOptimizedBvh() { return (btOptimizedBvh*)gOptimizedBvh; }
...
#else
    return btSelect((unsigned)((aabbMin1[0] <= aabbMax2[0]) & (aabbMax1[0] >= aabbMin2[0]) & (aabbMin1[2] <= aabbMax2[2]) & (aabbMax1[2] >= aabbMin2[2]) & (aabbMin1[1] <= aabbMax2[1]) & (aabbMax1[1] >= aabbMin2[1])), 1, 0);
#endif
}
#else
