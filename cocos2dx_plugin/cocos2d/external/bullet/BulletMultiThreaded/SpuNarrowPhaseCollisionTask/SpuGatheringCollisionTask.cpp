...
ATTRIBUTE_ALIGNED16(struct) CollisionTask_LocalStoreMemory {
    ...
    btCollisionObject* getColObj0() { return m_lsColObj0Ptr; }
    btCollisionObject* getColObj1() { return m_lsColObj1Ptr; }
    btBroadphasePair* getBroadphasePairPtr() { return m_pairsPointer; }
    SpuContactManifoldCollisionAlgorithm* getlocalCollisionAlgorithm() { return m_lsCollisionAlgorithmPtr; }
    btPersistentManifold* getContactManifoldPtr() { return m_lsManifoldPtr; }
};
#if defined(__CELLOS_LV2__) || defined(USE_LIBSPE2)
ATTRIBUTE_ALIGNED16(CollisionTask_LocalStoreMemory gLocalStoreMemory);
void* createCollisionLocalStoreMemory() { return &gLocalStoreMemory; }
...
ATTRIBUTE_ALIGNED16(class) spuNodeCallback : public btNodeOverlapCallback {
   ...
   public:
    spuNodeCallback(SpuCollisionPairInput * wuInput, CollisionTask_LocalStoreMemory * lsMemPtr, SpuContactResult & spuContacts) : m_wuInput(wuInput), m_spuContacts(spuContacts), m_lsMemPtr(lsMemPtr) {}
    virtual void processNode(int subPart, int triangleIndex) {
        ...
        if (m_lsMemPtr->bvhShapeData.gIndexMesh.m_indexType == PHY_SHORT) {
            ...
            small_cache_read_triple(&tmpIndices[0], (ppu_address_t) & indexBasePtr[0], &tmpIndices[1], (ppu_address_t) & indexBasePtr[1], &tmpIndices[2], (ppu_address_t) & indexBasePtr[2], sizeof(unsigned short int));
            ...
            small_cache_read_triple(&m_lsMemPtr->spuIndices[0], (ppu_address_t) & indexBasePtr[0], &m_lsMemPtr->spuIndices[1], (ppu_address_t) & indexBasePtr[1], &m_lsMemPtr->spuIndices[2], (ppu_address_t) & indexBasePtr[2], sizeof(int));
        }
        ...
        for (int j = 2; btLikely(j >= 0); j--) {
            ...
            small_cache_read_triple(&spuUnscaledVertex[0], (ppu_address_t) & graphicsbasePtr[0], &spuUnscaledVertex[1], (ppu_address_t) & graphicsbasePtr[1], &spuUnscaledVertex[2], (ppu_address_t) & graphicsbasePtr[2], sizeof(btScalar));
            m_tmpTriangleShape.getVertexPtr(j).setValue(spuUnscaledVertex[0] * meshScaling.getX(), spuUnscaledVertex[1] * meshScaling.getY(), spuUnscaledVertex[2] * meshScaling.getZ());
...
void ProcessConvexPlaneSpuCollision(SpuCollisionPairInput* wuInput, CollisionTask_LocalStoreMemory* lsMemPtr, SpuContactResult& spuContacts) {
    ...
    spuContacts.setContactInfo(spuManifold, manifoldAddress, lsMemPtr->getColObj0()->getWorldTransform(), lsMemPtr->getColObj1()->getWorldTransform(), lsMemPtr->getColObj0()->getRestitution(), lsMemPtr->getColObj1()->getRestitution(),
                               lsMemPtr->getColObj0()->getFriction(), lsMemPtr->getColObj1()->getFriction(), wuInput->m_isSwapped);
        ...
        for (int i = 0; i < numBatch;) {
            ...
            for (int j = 0; j < nextBatch; j++) {
                ...
                if (overlap) {
                    ...
                    cellDmaWaitTagStatusAll(DMA_MASK(2));
                    /* Walk this subtree */
                    spuWalkStacklessQuantizedTree(&nodeCallback, quantizedQueryAabbMin, quantizedQueryAabbMax, &lsMemPtr->bvhShapeData.gSubtreeNodes[0], 0, subtree.m_subtreeSize);
                }
...
void handleCollisionPair(SpuCollisionPairInput& collisionPairInput, CollisionTask_LocalStoreMemory& lsMem, SpuContactResult& spuContacts, ppu_address_t collisionShape0Ptr, void* collisionShape0Loc, ppu_address_t collisionShape1Ptr,
                         void* collisionShape1Loc, bool dmaShapes = true) {
    if (btBroadphaseProxy::isConvex(collisionPairInput.m_shapeType0) && btBroadphaseProxy::isConvex(collisionPairInput.m_shapeType1)) {
        if (dmaShapes) {
    ...
    } else if (btBroadphaseProxy::isCompound(collisionPairInput.m_shapeType0) && btBroadphaseProxy::isCompound(collisionPairInput.m_shapeType1)) {
        // snPause();
        dmaCollisionShape(collisionShape0Loc, collisionShape0Ptr, 1, collisionPairInput.m_shapeType0);
        dmaCollisionShape(collisionShape1Loc, collisionShape1Ptr, 2, collisionPairInput.m_shapeType1);
        cellDmaWaitTagStatusAll(DMA_MASK(1) | DMA_MASK(2));
        ...
        for (int i = 0; i < childShapeCount0; ++i) {
            ...
            for (int j = 0; j < childShapeCount1; ++j) {
                ...
                handleCollisionPair(cinput, lsMem, spuContacts, (ppu_address_t)childShape0.m_childShape, lsMem.compoundShapeData[0].gSubshapeShape[i], (ppu_address_t)childShape1.m_childShape, lsMem.compoundShapeData[1].gSubshapeShape[j], false);
            }
        }
        ...
        for (int i = 0; i < childShapeCount; ++i) {
            ...
            handleCollisionPair(cinput, lsMem, spuContacts, (ppu_address_t)childShape.m_childShape, lsMem.compoundShapeData[0].gSubshapeShape[i], collisionShape1Ptr, collisionShape1Loc, false);
        }
        ...
        for (int i = 0; i < childShapeCount; ++i) {
            ...
            handleCollisionPair(cinput, lsMem, spuContacts, collisionShape0Ptr, collisionShape0Loc, (ppu_address_t)childShape.m_childShape, lsMem.compoundShapeData[0].gSubshapeShape[i], false);
        }
    } else {
        // a non-convex shape is involved
        bool handleConvexConcave = false;
        // snPause();
        if (btBroadphaseProxy::isConcave(collisionPairInput.m_shapeType0) && btBroadphaseProxy::isConvex(collisionPairInput.m_shapeType1)) {
            // Swap stuff
        ...
        if (btBroadphaseProxy::isConvex(collisionPairInput.m_shapeType0) && btBroadphaseProxy::isConcave(collisionPairInput.m_shapeType1)) {
...
#define USE_DEDICATED_BOX_BOX 1
                                    ...
                                    bool boxbox = ((lsMem.getlocalCollisionAlgorithm()->getShapeType0() == BOX_SHAPE_PROXYTYPE) && (lsMem.getlocalCollisionAlgorithm()->getShapeType1() == BOX_SHAPE_PROXYTYPE));
                                    if (boxbox) {
                                        ...
                                        ppu_address_t manifoldAddress = (ppu_address_t)manifold;
                                        spuContacts.setContactInfo(spuManifold, manifoldAddress, lsMem.getColObj0()->getWorldTransform(), lsMem.getColObj1()->getWorldTransform(), lsMem.getColObj0()->getRestitution(),
                                                                   lsMem.getColObj1()->getRestitution(), lsMem.getColObj0()->getFriction(), lsMem.getColObj1()->getFriction(), collisionPairInput.m_isSwapped);
                                        // float distance=0.f;
                                        btVector3 normalInB;
                                        if (  //!gUseEpa &&
#ifdef USE_SEPDISTANCE_UTIL
...
// float distanceThreshold = 0.f;
                                                ...
                                                BoxPoint ptA, ptB;
                                                float dist = boxBoxDistance(n, ptA, ptB, boxA, trA, boxB, trB, distanceThreshold);
                                                //                                                float distance = boxBoxDistance(resultNormal,resultClosestBoxPointA,resultClosestBoxPointB,  boxA, transformA,
                                                //boxB,transformB,distanceThreshold);
                                                ...
                                                if (dist < spuManifold->getContactBreakingThreshold()) {
                                                    ...
                                                    spuContacts.addContactPoint(normalInB, pointOnB, dist);
                                                }
                                            }
#else
                                            {
                                                ...
                                                struct SpuBridgeContactCollector : public btDiscreteCollisionDetectorInterface::Result {
                                                    SpuContactResult& m_spuContacts;
                                                    virtual void setShapeIdentifiersA(int partId0, int index0) { m_spuContacts.setShapeIdentifiersA(partId0, index0); }
                                                    virtual void setShapeIdentifiersB(int partId1, int index1) { m_spuContacts.setShapeIdentifiersB(partId1, index1); }
                                                    virtual void addContactPoint(const btVector3& normalOnBInWorld, const btVector3& pointInWorld, btScalar depth) { m_spuContacts.addContactPoint(normalOnBInWorld, pointInWorld, depth); }
                                                    SpuBridgeContactCollector(SpuContactResult& spuContacts) : m_spuContacts(spuContacts) {}
                                                };
...
#endif  // USE_DEDICATED_BOX_BOX
                                    {
                                        if (
                                            ...
                                            handleCollisionPair(collisionPairInput, lsMem, spuContacts, (ppu_address_t)lsMem.getColObj0()->getCollisionShape(), &lsMem.gCollisionShapes[0].collisionShape,
