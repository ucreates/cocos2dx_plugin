...
#include "OpenCLC10/SolveCollisionsAndUpdateVelocities.cl"
    ...
    : m_queue(queue),
                  m_clClothIdentifier(queue, ctx, &m_clothIdentifier, false), m_clVertexPosition(queue, ctx, &m_vertexPosition, false), m_clVertexPreviousPosition(queue, ctx, &m_vertexPreviousPosition, false),
                  m_clVertexVelocity(queue, ctx, &m_vertexVelocity, false), m_clVertexForceAccumulator(queue, ctx, &m_vertexForceAccumulator, false), m_clVertexNormal(queue, ctx, &m_vertexNormal, false),
                  m_clVertexInverseMass(queue, ctx, &m_vertexInverseMass, false), m_clVertexArea(queue, ctx, &m_vertexArea, false), m_clVertexTriangleCount(queue, ctx, &m_vertexTriangleCount, false) {
}
btSoftBodyVertexDataOpenCL::~btSoftBodyVertexDataOpenCL() {}
bool btSoftBodyVertexDataOpenCL::onAccelerator() { return m_onGPU; }
...
void btSoftBodyLinkDataOpenCL::setLinkAt(const LinkDescription &link, int linkIndex) {
...
bool btSoftBodyLinkDataOpenCL::onAccelerator() { return m_onGPU; }
...
btSoftBodyTriangleDataOpenCL::btSoftBodyTriangleDataOpenCL(cl_command_queue queue, cl_context ctx)
    : m_queue(queue), m_clVertexIndices(queue, ctx, &m_vertexIndices, false), m_clArea(queue, ctx, &m_area, false), m_clNormal(queue, ctx, &m_normal, false) {}
...
bool btSoftBodyTriangleDataOpenCL::onAccelerator() { return m_onGPU; }
...
btOpenCLSoftBodySolver::~btOpenCLSoftBodySolver() { releaseKernels(); }
...
void btOpenCLSoftBodySolver::optimize(btAlignedObjectArray<btSoftBody *> &softBodies, bool forceUpdate) {
    if (forceUpdate || m_softBodySet.size() != softBodies.size()) {
        ...
        for (int softBodyIndex = 0; softBodyIndex < softBodies.size(); ++softBodyIndex) {
            ...
            if (piterations > maxPiterations) maxPiterations = piterations;
            int viterations = softBody->m_cfg.viterations;
            if (viterations > maxViterations) maxViterations = viterations;
            // zero mass
            for (int vertex = 0; vertex < numVertices; ++vertex) {
...
Vectormath::Aos::Vector3 btOpenCLSoftBodySolver::ProjectOnAxis(const Vectormath::Aos::Vector3 &v, const Vectormath::Aos::Vector3 &a) { return a * Vectormath::Aos::dot(v, a); }
...
float btOpenCLSoftBodySolver::computeTriangleArea(const Vectormath::Aos::Point3 &vertex0, const Vectormath::Aos::Point3 &vertex1, const Vectormath::Aos::Point3 &vertex2) {
...
void btOpenCLSoftBodySolver::prepareCollisionConstraints() {
    ...
    if (!m_perClothCollisionObjects.size()) return;
...
// Build the program with 'mad' Optimization option
    ...
    printf("ready. \n");
    delete[] compileFlags;
    if (!kernel) m_kernelCompilationFailures++;
    return kernel;
}
void btOpenCLSoftBodySolver::predictMotion(float timeStep) {
    ...
    if (m_bUpdateAnchoredNodePos) {
        ...
        if (m_anchorNodeInfoArray.size() > 0) m_clAnchorPosition.changedOnCPU();
...
bool btOpenCLSoftBodySolver::buildShaders() {
    ...
    m_solveCollisionsAndUpdateVelocitiesKernel =
        m_currentCLFunctions->compileCLKernelFromString(SolveCollisionsAndUpdateVelocitiesCLString, "SolveCollisionsAndUpdateVelocitiesKernel", additionalMacros, "OpenCLC10/SolveCollisionsAndUpdateVelocities.cl");
    ...
    if (m_currentCLFunctions->getKernelCompilationFailures() == 0) m_shadersInitialized = true;
    return m_shadersInitialized;
}
