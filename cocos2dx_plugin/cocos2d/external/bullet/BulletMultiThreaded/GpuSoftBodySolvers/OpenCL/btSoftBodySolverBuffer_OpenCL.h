...
class btOpenCLBuffer {
   public:
    ...
    bool createBuffer(cl_mem* preexistingBuffer = 0) {
        ...
        if (preexistingBuffer) {
            ...
            if (size == 0) size = sizeof(ElementType);
   ...
   public:
    btOpenCLBuffer(cl_command_queue commandQue, cl_context ctx, btAlignedObjectArray<ElementType>* CPUBuffer, bool readOnly)
        : m_cqCommandQue(commandQue), m_clContext(ctx), m_buffer(0), m_CPUBuffer(CPUBuffer), m_gpuSize(0), m_onGPU(false), m_readOnlyOnGPU(readOnly), m_allocated(false) {}
    ~btOpenCLBuffer() { clReleaseMemObject(m_buffer); }
    bool moveToGPU() {
        cl_int err;
        ...
        if (!m_onGPU && m_CPUBuffer->size() > 0) {
            ...
            size_t size = m_CPUBuffer->size() * sizeof(ElementType);
            err = clEnqueueWriteBuffer(m_cqCommandQue, m_buffer, CL_FALSE, 0, size, &((*m_CPUBuffer)[0]), 0, 0, 0);
            if (err != CL_SUCCESS) {
    ...
    bool moveFromGPU() {
        ...
        if (m_CPUBuffer->size() > 0) {
            if (m_onGPU && !m_readOnlyOnGPU) {
                size_t size = m_CPUBuffer->size() * sizeof(ElementType);
                err = clEnqueueReadBuffer(m_cqCommandQue, m_buffer, CL_TRUE, 0, size, &((*m_CPUBuffer)[0]), 0, 0, 0);
                if (err != CL_SUCCESS) {
    ...
    virtual void changedOnCPU() { m_onGPU = false; }
};  // class btOpenCLBuffer
