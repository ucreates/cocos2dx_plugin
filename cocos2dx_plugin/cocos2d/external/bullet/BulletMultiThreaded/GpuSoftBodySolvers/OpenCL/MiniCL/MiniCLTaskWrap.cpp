...
float mydot3a(float4 a, float4 b) { return a.x * b.x + a.y * b.y + a.z * b.z; }
typedef struct {
    int firstObject;
    int endObject;
...
__kernel void SolveCollisionsAndUpdateVelocitiesKernel(const int numNodes, const float isolverdt, __global int *g_vertexClothIdentifier, __global float4 *g_vertexPreviousPositions, __global float *g_perClothFriction,
                                                       __global float *g_clothDampingFactor, __global CollisionObjectIndices *g_perClothCollisionObjectIndices, __global CollisionShapeDescription *g_collisionObjectDetails,
                                                       __global float4 *g_vertexForces, __global float4 *g_vertexVelocities, __global float4 *g_vertexPositions GUID_ARG) {
            ...
            for (int collision = collisionObjectIndices.firstObject; collision < collisionObjectIndices.endObject; ++collision) {
                ...
                if (shapeDescription.collisionShapeType == CAPSULE_SHAPE_PROXYTYPE) {
                    ...
                    if (dist < minDistance) {
                        ...
                        if (approachSpeed <= 0.0f) forceOnVertex -= frictionVector;
                    }
                }
            }
        }
