...
class DXFunctions {
   public:
    ...
    DXFunctions(ID3D11Device *dx11Device, ID3D11DeviceContext *dx11Context, CompileFromMemoryFunc dx11CompileFromMemory) : m_dx11Device(dx11Device), m_dx11Context(dx11Context), m_dx11CompileFromMemory(dx11CompileFromMemory) {}
    class KernelDesc {
       protected:
       public:
...
class btDX11SoftBodySolver : public btSoftBodySolver {
   ...
   public:
    ...
    class btAcceleratedSoftBodyInterface {
       ...
       public:
        btAcceleratedSoftBodyInterface(btSoftBody *softBody) : m_softBody(softBody) {
            m_numVertices = 0;
            m_maxVertices = 0;
            m_numTriangles = 0;
            m_maxTriangles = 0;
        ...
        int getFirstTriangle() const { return m_firstTriangle; }
        /**
        ...
        void setMaxVertices(int maxVertices) { m_maxVertices = maxVertices; }
        void setMaxTriangles(int maxTriangles) { m_maxTriangles = maxTriangles; }
        void setFirstVertex(int firstVertex) { m_firstVertex = firstVertex; }
        void setFirstTriangle(int firstTriangle) { m_firstTriangle = firstTriangle; }
        void setMaxLinks(int maxLinks) { m_maxLinks = maxLinks; }
        void setNumLinks(int numLinks) { m_numLinks = numLinks; }
        void setFirstLink(int firstLink) { m_firstLink = firstLink; }
        int getMaxLinks() { return m_maxLinks; }
        int getNumLinks() { return m_numLinks; }
        int getFirstLink() { return m_firstLink; }
        btSoftBody *getSoftBody() { return m_softBody; }
    };
   ...
   public:
    ...
    virtual SolverTypes getSolverType() const { return DX_SOLVER; }
    void setEnableUpdateBounds(bool enableBounds) { m_enableUpdateBounds = enableBounds; }
    bool getEnableUpdateBounds() const { return m_enableUpdateBounds; }
...
class btSoftBodySolverOutputDXtoDX : public btSoftBodySolverOutputDXtoCPU {
   protected:
    ...
    void releaseKernels();
   public:
    btSoftBodySolverOutputDXtoDX(ID3D11Device *dx11Device, ID3D11DeviceContext *dx11Context, DXFunctions::CompileFromMemoryFunc dx11CompileFromMemory = &D3DX11CompileFromMemory) : dxFunctions(dx11Device, dx11Context, dx11CompileFromMemory) {
        m_shadersInitialized = false;
    }
    ~btSoftBodySolverOutputDXtoDX() { releaseKernels(); }
    /** Output current computed vertex data to the vertex buffers for all cloths in the solver. */
    virtual void copySoftBodyToVertexBuffer(const btSoftBody *const softBody, btVertexBufferDescriptor *vertexBuffer);
};
