...
class btDX11Buffer {
   protected:
    ...
    bool createBuffer(ID3D11Buffer* preexistingBuffer = 0) {
        ...
        if (m_readOnlyOnGPU) {
            ...
            if (srvbuffer_desc.Buffer.ElementWidth == 0) srvbuffer_desc.Buffer.ElementWidth = 1;
            ...
            if (uavbuffer_desc.Buffer.NumElements == 0) uavbuffer_desc.Buffer.NumElements = 1;
   ...
   public:
    ...
    bool moveToGPU() {
        ...
        if ((m_CPUBuffer->size() > m_gpuSize)) m_onGPU = false;
    ...
    bool moveFromGPU() {
        if (m_CPUBuffer->size() > 0) {
            if (m_onGPU && !m_readOnlyOnGPU) {
                ...
                destRegion.back = 1;
                destRegion.right = (m_CPUBuffer->size()) * sizeof(ElementType);
                m_d3dDeviceContext->CopySubresourceRegion(m_readBackBuffer, 0, 0, 0, 0, m_Buffer, 0, &destRegion);
