...
#include "HLSL/solveCollisionsAndUpdateVelocitiesSIMDBatched.hlsl"
    ...
    : m_d3dDevice(d3dDevice),
            m_d3dDeviceContext(d3dDeviceContext), m_wavefrontSize(WAVEFRONT_SIZE), m_linksPerWorkItem(LINKS_PER_SIMD_LANE), m_maxBatchesWithinWave(0), m_maxLinksPerWavefront(m_wavefrontSize * m_linksPerWorkItem), m_numWavefronts(0), m_maxVertex(0),
            m_dx11NumBatchesAndVerticesWithinWaves(d3dDevice, d3dDeviceContext, &m_numBatchesAndVerticesWithinWaves, true), m_dx11WavefrontVerticesGlobalAddresses(d3dDevice, d3dDeviceContext, &m_wavefrontVerticesGlobalAddresses, true),
            m_dx11LinkVerticesLocalAddresses(d3dDevice, d3dDeviceContext, &m_linkVerticesLocalAddresses, true), m_dx11LinkStrength(d3dDevice, d3dDeviceContext, &m_linkStrength, true),
            m_dx11LinksMassLSC(d3dDevice, d3dDeviceContext, &m_linksMassLSC, true), m_dx11LinksRestLengthSquared(d3dDevice, d3dDeviceContext, &m_linksRestLengthSquared, true),
            m_dx11LinksRestLength(d3dDevice, d3dDeviceContext, &m_linksRestLength, true), m_dx11LinksMaterialLinearStiffnessCoefficient(d3dDevice, d3dDeviceContext, &m_linksMaterialLinearStiffnessCoefficient, true) {
    m_d3dDevice = d3dDevice;
...
void btSoftBodyLinkDataDX11SIMDAware::setLinkAt(const btSoftBodyLinkData::LinkDescription &link, int linkIndex) {
    ...
    if (link.getVertex0() > m_maxVertex) m_maxVertex = link.getVertex0();
    if (link.getVertex1() > m_maxVertex) m_maxVertex = link.getVertex1();
    // Set the link index correctly for initialisation
    m_linkAddresses[linkIndex] = linkIndex;
}
bool btSoftBodyLinkDataDX11SIMDAware::onAccelerator() { return m_onGPU; }
...
btDX11SIMDAwareSoftBodySolver::btDX11SIMDAwareSoftBodySolver(ID3D11Device *dx11Device, ID3D11DeviceContext *dx11Context, DXFunctions::CompileFromMemoryFunc dx11CompileFromMemory)
    : btDX11SoftBodySolver(dx11Device, dx11Context, dx11CompileFromMemory), m_linkData(m_dx11Device, m_dx11Context) {
...
btDX11SIMDAwareSoftBodySolver::~btDX11SIMDAwareSoftBodySolver() { releaseKernels(); }
...
bool btDX11SIMDAwareSoftBodySolver::buildShaders() {
    ...
    D3D10_SHADER_MACRO solvePositionsMacros[6] = {"MAX_NUM_VERTICES_PER_WAVE",  maxVerticesPerWavefront,  "MAX_BATCHES_PER_WAVE", maxBatchesPerWavefront, "WAVEFRONT_SIZE", waveFrontSize,
                                                  "WAVEFRONT_BLOCK_MULTIPLIER", waveFrontBlockMultiplier, "BLOCK_SIZE",           blockSize,              0,                0};
    ...
    if (!solvePositionsFromLinksKernel.constBuffer) returnVal = false;
...
static void removeFromVector(btAlignedObjectArray<T> &vectorToUpdate, int indexToRemove) {
    ...
    if (currentSize > 0) vectorToUpdate.resize(currentSize - 1);
}
/**
...
static void insertUniqueAndOrderedIntoVector(btAlignedObjectArray<T> &vectorToUpdate, T element) {
    ...
    if (index == vectorToUpdate.size() || vectorToUpdate[index] != element) insertAtIndex(vectorToUpdate, index, element);
}
...
static void computeBatchingIntoWavefronts(btSoftBodyLinkData &linkData, int wavefrontSize, int linksPerWorkItem, int maxLinksPerWavefront, btAlignedObjectArray<btAlignedObjectArray<int>> &linksForWavefronts,
        ...
        while (linksProcessed < linkData.getNumLinks() && linksForWavefront.size() < maxLinksPerWavefront) {
            ...
            if (verticesToProcess.size() > 0) {
                ...
                if (linksProcessed < linkData.getNumLinks()) {
                    int searchLink = 0;
                    while (processedLink[searchLink]) searchLink++;
            ...
            while (batch < batchesWithinWave.size() && !placed) {
                ...
                if (batchesWithinWave[batch].size() >= wavefrontSize) {
                    ...
                    for (int link2 = 0; link2 < batchesWithinWave[batch].size(); ++link2) {
                        ...
                        if (vertices.vertex0 == vertices2.vertex0 || vertices.vertex1 == vertices2.vertex0 || vertices.vertex0 == vertices2.vertex1 || vertices.vertex1 == vertices2.vertex1) {
                            foundSharedVertex = true;
                            break;
                        }
                    }
