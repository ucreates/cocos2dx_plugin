...
#include "BulletCollision/CollisionDispatch/btCollisionObjectWrapper.h"
    ...
    : m_dx11Links(d3dDevice, d3dDeviceContext, &m_links, false),
            m_dx11LinkStrength(d3dDevice, d3dDeviceContext, &m_linkStrength, false), m_dx11LinksMassLSC(d3dDevice, d3dDeviceContext, &m_linksMassLSC, false), m_dx11LinksRestLengthSquared(d3dDevice, d3dDeviceContext, &m_linksRestLengthSquared, false),
            m_dx11LinksCLength(d3dDevice, d3dDeviceContext, &m_linksCLength, false), m_dx11LinksLengthRatio(d3dDevice, d3dDeviceContext, &m_linksLengthRatio, false), m_dx11LinksRestLength(d3dDevice, d3dDeviceContext, &m_linksRestLength, false),
...
bool btSoftBodyLinkDataDX11::onAccelerator() { return m_onGPU; }
...
void btSoftBodyLinkDataDX11::generateBatches() {
    ...
    for (int linkIndex = 0; linkIndex < numLinks; ++linkIndex) {
        ...
        if (vertex0 > maxVertex) maxVertex = vertex0;
        if (vertex1 > maxVertex) maxVertex = vertex1;
    }
    ...
    for (int linkIndex = 0; linkIndex < numLinks; ++linkIndex) {
        ...
        int colour = 0;
        while (colourListVertex0.findLinearSearch(colour) != colourListVertex0.size() || colourListVertex1.findLinearSearch(colour) != colourListVertex1.size()) ++colour;
    ...
    for (int batch = 0; batch < batchCounts.size(); ++batch) batchCounts[batch] = 0;
...
bool btSoftBodyVertexDataDX11::onAccelerator() { return m_onGPU; }
...
btSoftBodyTriangleDataDX11::btSoftBodyTriangleDataDX11(ID3D11Device *d3dDevice, ID3D11DeviceContext *d3dDeviceContext)
    : m_dx11VertexIndices(d3dDevice, d3dDeviceContext, &m_vertexIndices, false), m_dx11Area(d3dDevice, d3dDeviceContext, &m_area, false), m_dx11Normal(d3dDevice, d3dDeviceContext, &m_normal, false) {
    m_d3dDevice = d3dDevice;
    m_d3dDeviceContext = d3dDeviceContext;
}
...
bool btSoftBodyTriangleDataDX11::onAccelerator() { return m_onGPU; }
...
void btSoftBodyTriangleDataDX11::generateBatches() {
    ...
    for (int triangleIndex = 0; triangleIndex < numTriangles; ++triangleIndex) {
        ...
        if (vertex0 > maxVertex) maxVertex = vertex0;
        if (vertex1 > maxVertex) maxVertex = vertex1;
        if (vertex2 > maxVertex) maxVertex = vertex2;
    }
    int numVertices = maxVertex + 1;
    // Set of lists, one for each node, specifying which colours are connected
    // to that node.
    ...
    for (int triangleIndex = 0; triangleIndex < numTriangles; ++triangleIndex) {
        ...
        int colour = 0;
        while (colourListVertex0.findLinearSearch(colour) != colourListVertex0.size() || colourListVertex1.findLinearSearch(colour) != colourListVertex1.size() || colourListVertex2.findLinearSearch(colour) != colourListVertex2.size()) {
            ++colour;
        }
        // i should now be the minimum colour in neither list
...
btDX11SoftBodySolver::~btDX11SoftBodySolver() { releaseKernels(); }
...
bool btDX11SoftBodySolver::checkInitialized() {
    if (!m_shadersInitialized)
        if (buildShaders()) m_shadersInitialized = true;
...
Vectormath::Aos::Vector3 btDX11SoftBodySolver::ProjectOnAxis(const Vectormath::Aos::Vector3 &v, const Vectormath::Aos::Vector3 &a) { return a * Vectormath::Aos::dot(v, a); }
...
float btDX11SoftBodySolver::computeTriangleArea(const Vectormath::Aos::Point3 &vertex0, const Vectormath::Aos::Point3 &vertex1, const Vectormath::Aos::Point3 &vertex2) {
...
void btDX11SoftBodySolver::prepareCollisionConstraints() {
    ...
    class QuickSortCompare {
       public:
        bool operator()(const CollisionShapeDescription &a, const CollisionShapeDescription &b) const { return (a.softBodyIdentifier < b.softBodyIdentifier); }
    };
...
btDX11SoftBodySolver::btAcceleratedSoftBodyInterface *btDX11SoftBodySolver::findSoftBodyInterface(const btSoftBody *const softBody) {
    for (int softBodyIndex = 0; softBodyIndex < m_softBodySet.size(); ++softBodyIndex) {
        btAcceleratedSoftBodyInterface *softBodyInterface = m_softBodySet[softBodyIndex];
        if (softBodyInterface->getSoftBody() == softBody) return softBodyInterface;
    }
    return 0;
}
...
int btDX11SoftBodySolver::findSoftBodyIndex(const btSoftBody *const softBody) {
    for (int softBodyIndex = 0; softBodyIndex < m_softBodySet.size(); ++softBodyIndex) {
        btAcceleratedSoftBodyInterface *softBodyInterface = m_softBodySet[softBodyIndex];
        if (softBodyInterface->getSoftBody() == softBody) return softBodyIndex;
    }
    return 1;
}
...
bool btSoftBodySolverOutputDXtoDX::buildShaders() {
    ...
    releaseKernels();
    bool returnVal = true;
    if (m_shadersInitialized) return true;
    ...
    if (!outputToVertexArrayWithNormalsKernel.constBuffer) returnVal = false;
    ...
    if (!outputToVertexArrayWithoutNormalsKernel.constBuffer) returnVal = false;
    if (returnVal) m_shadersInitialized = true;
    return returnVal;
}
...
DXFunctions::KernelDesc DXFunctions::compileComputeShaderFromString(const char *shaderString, const char *shaderName, int constBufferSize, D3D10_SHADER_MACRO *compileMacros) {
    ...
    hr = m_dx11CompileFromMemory(shaderString, strlen(shaderString), shaderName, compileMacros, NULL, shaderName, cs5String, D3D10_SHADER_ENABLE_STRICTNESS, NULL, NULL, &pBlob, &pErrorBlob, NULL);
    if (FAILED(hr)) {
        if (pErrorBlob) {
...
bool btDX11SoftBodySolver::buildShaders() {
    ...
    if (!prepareLinksKernel.constBuffer) returnVal = false;
    ...
    if (!updatePositionsFromVelocitiesKernel.constBuffer) returnVal = false;
    ...
    if (!vSolveLinksKernel.constBuffer) returnVal = false;
    ...
    if (!updateVelocitiesFromPositionsWithVelocitiesKernel.constBuffer) returnVal = false;
    ...
    if (!updateVelocitiesFromPositionsWithoutVelocitiesKernel.constBuffer) returnVal = false;
    integrateKernel = dxFunctions.compileComputeShaderFromString(IntegrateHLSLString, "IntegrateKernel", sizeof(IntegrateCB));
    if (!integrateKernel.constBuffer) returnVal = false;
    ...
    if (!applyForcesKernel.constBuffer) returnVal = false;
    ...
    if (!solveCollisionsAndUpdateVelocitiesKernel.constBuffer) returnVal = false;
    ...
    if (!resetNormalsAndAreasKernel.constBuffer) returnVal = false;
    ...
    if (!normalizeNormalsAndAreasKernel.constBuffer) returnVal = false;
    ...
    if (!updateSoftBodiesKernel.constBuffer) returnVal = false;
    ...
    if (!computeBoundsKernel.constBuffer) returnVal = false;
    if (returnVal) m_shadersInitialized = true;
    return returnVal;
}
...
void btDX11SoftBodySolver::predictMotion(float timeStep) {
    ...
    if (m_enableUpdateBounds) updateBounds();
