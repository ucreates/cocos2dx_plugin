...
struct btInternalVertexPair {
    ...
    bool equals(const btInternalVertexPair& other) const { return m_v0 == other.m_v0 && m_v1 == other.m_v1; }
};
    ...
    for (int i = 0; i < m_faces.size(); i++) {
        ...
        for (int j = 0; j < NbTris; j++) {
            ...
            for (int p = 0; p < m_uniqueEdges.size(); p++) {
                if (IsAlmostZero(m_uniqueEdges[p] - edge) || IsAlmostZero(m_uniqueEdges[p] + edge)) {
                    found = true;
                    break;
                }
            }
