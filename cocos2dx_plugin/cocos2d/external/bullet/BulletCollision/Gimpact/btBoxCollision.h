...
SIMD_FORCE_INLINE btScalar bt_mat3_dot_col(const btMatrix3x3& mat, const btVector3& vec3, int colindex) { return vec3[0] * mat[0][colindex] + vec3[1] * mat[1][colindex] + vec3[2] * mat[2][colindex]; }
...
ATTRIBUTE_ALIGNED16(class) BT_BOX_BOX_TRANSFORM_CACHE {
   public:
    ...
    SIMD_FORCE_INLINE btVector3 transform(const btVector3 & point) const { return point.dot3(m_R1to0[0], m_R1to0[1], m_R1to0[2]) + m_T1to0; }
};
...
ATTRIBUTE_ALIGNED16(class) btAABB {
   public:
    ...
    SIMD_FORCE_INLINE void appy_transform(const btTransform & trans) {
        ...
        // Compute new center
        center = trans(center);
        btVector3 textends = extends.dot3(trans.getBasis().getRow(0).absolute(), trans.getBasis().getRow(1).absolute(), trans.getBasis().getRow(2).absolute());
    ...
    SIMD_FORCE_INLINE void appy_transform_trans_cache(const BT_BOX_BOX_TRANSFORM_CACHE & trans) {
        ...
        btVector3 textends = extends.dot3(trans.m_R1to0.getRow(0).absolute(), trans.m_R1to0.getRow(1).absolute(), trans.m_R1to0.getRow(2).absolute());
        m_min = center - textends;
        m_max = center + textends;
    }
    //! Merges a Box
