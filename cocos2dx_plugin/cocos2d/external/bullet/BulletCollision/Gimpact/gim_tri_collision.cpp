...
class GIM_TRIANGLE_CALCULATION_CACHE {
    ...
    */
    SIMD_FORCE_INLINE GUINT clip_triangle(const btVector4 &tri_plane, const btVector3 *tripoints, const btVector3 *srcpoints, btVector3 *clip_points) {
        ...
        GUINT clipped_count = PLANE_CLIP_TRIANGLE3D(edgeplane, srcpoints[0], srcpoints[1], srcpoints[2], temp_points);
        if (clipped_count == 0) return 0;
        // edge 1
        EDGE_PLANE(tripoints[1], tripoints[2], tri_plane, edgeplane);
        clipped_count = PLANE_CLIP_POLYGON3D(edgeplane, temp_points, clipped_count, temp_points1);
        if (clipped_count == 0) return 0;
        // edge 2
        EDGE_PLANE(tripoints[2], tripoints[0], tri_plane, edgeplane);
        clipped_count = PLANE_CLIP_POLYGON3D(edgeplane, temp_points1, clipped_count, clip_points);
    ...
    */
    SIMD_FORCE_INLINE GUINT cross_line_intersection_test() {
        ...
        if (!compute_intervals(du[0], du[1], du[2], du0du1, du0du2, tu_scale_e0, tu_scale_e1, tu_e0, tu_e1)) return 0;
        ...
        if (midpoint_u < midpoint_v) {
            ...
            {
                SEGMENT_COLLISION(tu_vertices[tu_e1], tu_vertices[(tu_e1 + 1) % 3], tv_vertices[tv_e0], tv_vertices[(tv_e0 + 1) % 3], closest_point_u, closest_point_v);
            ...
            {
                SEGMENT_COLLISION(tu_vertices[tu_e0], tu_vertices[(tu_e0 + 1) % 3], tv_vertices[tv_e1], tv_vertices[(tv_e1 + 1) % 3], closest_point_u, closest_point_v);
                edge_edge_dir = closest_point_u - closest_point_v;
                VEC_LENGTH(edge_edge_dir, distances[2]);
                edge_edge_dir *= 1.0f / distances[2];  // normalize
            } else {
...
bool GIM_TRIANGLE::collide_triangle_hard_test(const GIM_TRIANGLE &other, GIM_TRIANGLE_CONTACT_DATA &contact_data) const {
    GIM_TRIANGLE_CALCULATION_CACHE calc_cache;
    return calc_cache.triangle_collision(m_vertices[0], m_vertices[1], m_vertices[2], m_margin, other.m_vertices[0], other.m_vertices[1], other.m_vertices[2], other.m_margin, contact_data);
}
