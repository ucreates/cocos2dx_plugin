...
class JSStringWrapper {
    ...
    JSContext* cx;
   public:
    JSStringWrapper() { buffer = NULL; }
    JSStringWrapper(JSString* str, JSContext* cx) { set(str, cx); }
    JSStringWrapper(jsval val, JSContext* cx) { set(val, cx); }
    ~JSStringWrapper() {
        if (buffer) {
            // delete[] buffer;
            // buffer = NULL;
...
// JSFunctionWrapper
JSFunctionWrapper::JSFunctionWrapper(JSContext* cx, JSObject* jsthis, jsval fval) : _cx(cx), _jsthis(jsthis), _fval(fval) {
...
bool jsval_to_TProductInfo(JSContext* cx, JS::HandleValue v, TProductInfo* ret) {
    ...
    while (true) {
        ...
        if (!JS_NextProperty(cx, it, &idp) || !JS_IdToValue(cx, idp, &key)) return false;  // error
        if (key.isNullOrUndefined()) break;                                                // end of iteration
        if (!key.isString()) continue;                                                     // ignore integer properties
        JS::RootedValue value(cx);
        JS_GetPropertyById(cx, tmp, JS::RootedId(cx, idp), &value);
        if (!value.isString()) continue;  // ignore integer properties
...
bool jsval_to_TIAPDeveloperInfo(JSContext* cx, JS::HandleValue v, TIAPDeveloperInfo* ret) { return jsval_to_TProductInfo(cx, v, ret); }
bool jsval_to_TAdsDeveloperInfo(JSContext* cx, JS::HandleValue v, TAdsDeveloperInfo* ret) { return jsval_to_TProductInfo(cx, v, ret); }
bool jsval_to_TAdsInfo(JSContext* cx, JS::HandleValue v, TAdsInfo* ret) { return jsval_to_TProductInfo(cx, v, ret); }
bool jsval_to_TShareDeveloperInfo(JSContext* cx, JS::HandleValue v, TShareDeveloperInfo* ret) { return jsval_to_TProductInfo(cx, v, ret); }
bool jsval_to_TShareInfo(JSContext* cx, JS::HandleValue v, TShareInfo* ret) { return jsval_to_TProductInfo(cx, v, ret); }
bool jsval_to_TPaymentInfo(JSContext* cx, JS::HandleValue v, std::map<std::string, std::string>* ret) { return jsval_to_TProductInfo(cx, v, ret); }
bool jsval_to_TSocialDeveloperInfo(JSContext* cx, JS::HandleValue v, TSocialDeveloperInfo* ret) { return jsval_to_TProductInfo(cx, v, ret); }
bool jsval_to_TAchievementInfo(JSContext* cx, JS::HandleValue v, TAchievementInfo* ret) { return jsval_to_TProductInfo(cx, v, ret); }
bool jsval_to_TUserDeveloperInfo(JSContext* cx, JS::HandleValue v, TUserDeveloperInfo* ret) { return jsval_to_TProductInfo(cx, v, ret); }
...
bool jsval_to_StringMap(JSContext* cx, JS::HandleValue v, StringMap* ret) { return jsval_to_TProductInfo(cx, v, ret); }
...
jsval int32_to_jsval(JSContext* cx, int32_t number) { return INT_TO_JSVAL(number); }
...
jsval std_string_to_jsval(JSContext* cx, const std::string& v) { return c_string_to_jsval(cx, v.data()); }
