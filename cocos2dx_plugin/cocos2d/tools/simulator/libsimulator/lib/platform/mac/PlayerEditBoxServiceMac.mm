...
- (id)initWithFrame:(NSRect)frameRect editBox:(void *)editBox {
    ...
    if (self) {
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(onTextDidChanged:) name:NSControlTextDidEndEditingNotification object:nil];
    }
    return self;
}
...
PlayerEditBoxServiceMac::~PlayerEditBoxServiceMac() { [_sysEdit release]; }
...
void PlayerEditBoxServiceMac::setText(const std::string &text) { _sysEdit.textField.stringValue = [NSString stringWithUTF8String:text.c_str()]; }
...
void PlayerEditBoxServiceMac::setFormator(int formator) { CCLOG("Not support yet."); }
PLAYER_NS_END;