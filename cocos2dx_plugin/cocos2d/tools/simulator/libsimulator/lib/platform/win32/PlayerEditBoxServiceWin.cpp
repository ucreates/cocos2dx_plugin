...
PLAYER_NS_BEGIN
PlayerEditBoxServiceWin::PlayerEditBoxServiceWin(HWND hwnd) : _hfont(NULL) {
    _hwnd = hwnd;
...
void PlayerEditBoxServiceWin::setFont(const std::string &name, int size) {
    ...
    ReleaseDC(_hwnd, hdc);
    _hfont = CreateFont(size, 0, 0, 0, FW_NORMAL, FALSE, FALSE, FALSE, 0, OUT_DEFAULT_PRECIS, FW_NORMAL, DEFAULT_QUALITY, DEFAULT_PITCH, (LPCTSTR)u16name.c_str());
    if (!_hfont) {
