...
void onHelpAbout() { DialogBox(GetModuleHandle(NULL), MAKEINTRESOURCE(IDD_DIALOG_ABOUT), Director::getInstance()->getOpenGLView()->getWin32Window(), AboutDialogCallback); }
void shutDownApp() {
...
std::string getCurAppPath(void) {
    ...
    if (!GetModuleFileName(NULL, szAppDir, MAX_PATH)) return "";
    int nEnd = 0;
    for (int i = 0; szAppDir[i]; i++) {
        if (szAppDir[i] == '\\') nEnd = i;
    }
    szAppDir[nEnd] = 0;
...
SimulatorWin::SimulatorWin() : _app(nullptr), _hwnd(NULL), _hwndConsole(NULL), _writeDebugLogFile(nullptr) {}
...
#define MAX_COMMAND 1024  // lenth of commandLine is always beyond MAX_PATH
    ...
    BOOL success = CreateProcess(NULL, command,  // command line
...
void SimulatorWin::setupUI() {
    ...
    menuBar->addItem("DIRECTION_PORTRAIT_MENU", tr("Portrait"), "VIEW_MENU")->setChecked(_project.isPortraitFrame());
    menuBar->addItem("DIRECTION_LANDSCAPE_MENU", tr("Landscape"), "VIEW_MENU")->setChecked(_project.isLandscapeFrame());
    ...
    auto listener = EventListenerCustom::create(kAppEventDropName, [&project, app](EventCustom *event) {
        ...
        if (dropEvent) {
            ...
            if (FileUtils::getInstance()->isDirectoryExist(dirPath) && FileUtils::getInstance()->isFileExist(configFilePath)) {
                // parse config.json
...
void SimulatorWin::parseCocosProjectConfig(ProjectConfig &config) {
    ...
    if (!solutionDir.empty()) {
        ...
        if (nPos > 0) solutionDir = solutionDir.substr(0, nPos + 1);
