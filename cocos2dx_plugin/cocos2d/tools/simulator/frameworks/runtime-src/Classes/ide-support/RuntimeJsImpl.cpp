...
void RuntimeJsImpl::startScript(const std::string& path) { loadScriptFile(path); }
...
void RuntimeJsImpl::onPrecompile(const rapidjson::Document& dArgParse, rapidjson::Document& dReplyParse) {
    ...
    for (rapidjson::SizeType i = 0; i < objectfiles.Size(); i++) {
        ...
        sc->compileScript(objectfiles[i].GetString(), JS::RootedObject(gc, sc->getGlobalObject()), gc);
    }
    dReplyParse.AddMember("code", 0, dReplyParse.GetAllocator());
}
...
void RuntimeJsImpl::onRemove(const std::string& filename) { ScriptingCore::getInstance()->cleanScript(filename.c_str()); }
void RuntimeJsImpl::end() {
    ScriptEngineManager::destroyInstance();
    RuntimeProtocol::end();
}
// private
RuntimeJsImpl::RuntimeJsImpl() : _hasStarted(false) {}
